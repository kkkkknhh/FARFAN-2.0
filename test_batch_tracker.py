...
class BatchTestTracker:
        ...
        if self.RESULTS_FILE.exists():
            try:
                with open(self.RESULTS_FILE, 'r') as f:
                    data = json.load(f)
                    self.test_results = [TestResult(**r) for r in data.get('results', [])]
            except Exception as e:
    ...
    def record_edit(self, file_path: str, description: str = ""):
        ...
        print(f"  Batch {self.current_batch.batch_id}: {len(self.current_batch.edits)}/{self.BATCH_SIZE} edits")
    ...
    def run_tests(self, test_pattern: str = "test_*.py") -> TestResult:
        ...
        total_tests, passed_tests, failed_tests, skipped_tests = self._parse_unittest_output(output)
    ...
    def _extract_failures(self, output: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(pattern, output, re.DOTALL):
            ...
            failures.append({
                ...
                'module': test_class.split('.')[0] if '.' in test_class else test_class
            })
    ...
    def _generate_test_report(self, result: TestResult):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"  Passed:  {result.passed_tests} ({result.passed_tests/max(result.total_tests,1)*100:.1f}%)\n")
            ...
            if result.failures:
                ...
                for i, failure in enumerate(result.failures, 1):
                    f.write(f"{i}. {failure['type']}: {failure['test_class']}.{failure['test_name']}\n")
    ...
    def print_status(self):
        ...
        if self.test_results:
            ...
            print(f"  Tests: {last_result.passed_tests}/{last_result.total_tests} passed")
            
            if not last_result.success:
                print(f"  Failures: {last_result.failed_tests}")
                if last_result.failures:
                    print(f"  Failed modules: {', '.join(set(f['module'] for f in last_result.failures))}")
...
def main():
    ...
    record_parser.add_argument('-d', '--description', default='', help='Edit description')
    
    # Run tests command
    test_parser = subparsers.add_parser('test', help='Run test suite')
    test_parser.add_argument('-p', '--pattern', default='test_*.py', help='Test file pattern')
    
    # Status command
    subparsers.add_parser('status', help='Show tracker status')
    
    # Backup command
    backup_parser = subparsers.add_parser('backup', help='Create backup of current batch')
