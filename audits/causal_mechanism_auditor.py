...
class CausalMechanismAuditor:
    ...
    def audit_mechanism_necessity(
        ...
        for source, target in graph.edges():
            ...
            components_present = sum([has_entity, has_activity, has_budget, has_timeline])
            ...
            remediation = self._generate_necessity_remediation(
                link_id, missing_components
        ...
        overall_grade = QualityGrade.EXCELENTE if necessity_rate >= 0.8 else (
            QualityGrade.BUENO if necessity_rate >= 0.6 else (
                QualityGrade.REGULAR if necessity_rate >= 0.4 else 
                QualityGrade.INSUFICIENTE
            )
        )
    ...
    def _check_timeline(self, node_data: Dict[str, Any], text: str, node_id: str) -> bool:
    ...
    def _extract_activity_mentions(self, node_id: str, text: str) -> List[str]:
        ...
        mentions = []
        activity_pattern = r'(?:realiza|ejecuta|implementa|desarrolla)\s+(?:actividad|acción|proyecto)'
    ...
    def audit_root_cause_mapping(
        ...
        for node_id in graph.nodes():
            ...
            if node_type == 'programa' or 'diagnostic' in node_data.get('text', '').lower():
        ...
        for d2_node in d2_nodes:
            ...
            mapping_confidence = min(1.0, len(root_causes) / max(1, len(d1_nodes))) if d1_nodes else 0.0
    ...
    def audit_causal_proportionality(
        ...
        for source, target in graph.edges():
            ...
            original_posterior = edge_data.get('posterior_mean', edge_data.get('strength', 0.5))
    ...
    def generate_comprehensive_audit(
        ...
        # Run all four audits
        necessity_results = self.audit_mechanism_necessity(graph, text, inferred_mechanisms)
        ...
        summary = {
            'audit_point_2_1_mechanism_necessity': {
                ...
                'necessary_links': sum(1 for r in necessity_results.values() if r.is_necessary),
                'necessity_rate': sum(1 for r in necessity_results.values() if r.is_necessary) / len(necessity_results) if necessity_results else 0.0,
                'average_score': np.mean([r.necessity_score for r in necessity_results.values()]) if necessity_results else 0.0,
                'quality_distribution': self._count_quality_grades(necessity_results.values())
            },
            'audit_point_2_2_root_cause_mapping': {
                'total_activities': len(root_cause_results),
                'mapped_activities': sum(1 for r in root_cause_results.values() if r.root_causes),
                'average_coherence': np.mean([r.coherence_score for r in root_cause_results.values()]) if root_cause_results else 0.0,
                'meets_target': np.mean([r.coherence_score for r in root_cause_results.values()]) > 0.95 if root_cause_results else False,
                'quality_distribution': self._count_quality_grades(root_cause_results.values())
            },
            'audit_point_2_3_causal_proportionality': {
                'total_links': len(proportionality_results),
                'proportional_links': sum(1 for r in proportionality_results.values() if r.is_proportional),
                'capped_links': sum(1 for r in proportionality_results.values() if r.posterior_capped),
                'logical_jumps': sum(1 for r in proportionality_results.values() if r.has_logical_jump),
                'quality_distribution': self._count_quality_grades(proportionality_results.values())
            },
            'audit_point_2_4_activity_logic': {
                'total_activities': len(activity_logic_results),
                'complete_extractions': sum(1 for r in activity_logic_results.values() if r.extraction_accuracy == 1.0),
                'average_accuracy': np.mean([r.extraction_accuracy for r in activity_logic_results.values()]) if activity_logic_results else 0.0,
                'meets_target': np.mean([r.extraction_accuracy for r in activity_logic_results.values()]) >= 1.0 if activity_logic_results else False,
                'quality_distribution': self._count_quality_grades(activity_logic_results.values())
            }
        }
        ...
        return {
            'summary': summary,
            'audit_point_2_1_necessity': {k: v.to_dict() for k, v in necessity_results.items()},
            'audit_point_2_2_root_cause_mapping': {k: v.to_dict() for k, v in root_cause_results.items()},
            'audit_point_2_3_proportionality': {k: v.to_dict() for k, v in proportionality_results.items()},
            'audit_point_2_4_activity_logic': {k: v.to_dict() for k, v in activity_logic_results.items()}
        }
