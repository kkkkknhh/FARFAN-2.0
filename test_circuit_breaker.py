...
async def test_circuit_breaker_metrics():
    ...
    print(f"✓ Metrics tracked: {metrics.total_calls} total, "
...
async def test_resilient_validator_fail_open_policy():
    ...
    print(f"✓ Fail-open policy applied: score={result.score}, penalty={result.score_penalty}")
...
async def test_resilient_validator_metrics():
    ...
    data = PDMData(
        sector="agua",
        descripcion="Acueducto rural",
        indicadores_propuestos=["IND005"]
    )
    ...
    print(f"✓ Metrics: {metrics['total_calls']} calls, "
