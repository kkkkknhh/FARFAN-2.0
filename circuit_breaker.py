...
class CircuitBreaker:
    ...
    def __init__(self, name: str, config: Optional[CircuitBreakerConfig] = None):
        ...
        logger.info(f"CircuitBreaker '{name}' inicializado: {self.config.failure_threshold} fallos, {self.config.timeout}s timeout")
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == CircuitState.OPEN:
            if self._should_attempt_reset():
                self._transition(CircuitState.HALF_OPEN, "Timeout expired, testing recovery")
            else:
    ...
    def _on_success(self):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            logger.info(f"CircuitBreaker '{self.name}': Success in HALF_OPEN ({self.success_count}/{self.config.success_threshold})")
            
            if self.success_count >= self.config.success_threshold:
                self._transition(CircuitState.CLOSED, "Recovered after success threshold")
                self.failure_count = 0
                self.success_count = 0
    ...
    def _on_failure(self, exception: Exception):
        ...
        logger.warning(f"CircuitBreaker '{self.name}': Failure ({self.failure_count}/{self.config.failure_threshold}) - {exception}")
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.failure_count >= self.config.failure_threshold:
                self._transition(CircuitState.OPEN, f"Failure threshold reached: {self.failure_count} failures")
                self.opened_at = datetime.now()
    ...
    def reset(self):
        ...
        self.success_count = 0
        self.opened_at = None
        logger.info(f"CircuitBreaker '{self.name}' manually reset from {old_state.value}")
