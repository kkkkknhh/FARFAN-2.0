...
def example_basic_usage():
    ...
    container.register_singleton(
        DeviceConfig, 
        lambda: DeviceConfig(device='cpu')
    )
...
def example_testing_with_mocks():
    ...
    print(f"Builder has access to extractor: {isinstance(builder.extractor, MockPDFProcessor)}")
...
def example_automatic_dependency_injection():
    ...
    print(f"ServiceB has ServiceA: {isinstance(service_c.service_b.service_a, ServiceA)}")
    ...
    print(f"ServiceA is singleton (same instance): {service_c.service_a is service_c2.service_a}")
