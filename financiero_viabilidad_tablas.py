...
class PDETMunicipalPlanAnalyzer:
        ...
        try:
            ...
            for idx, table in enumerate(lattice_tables):
                if table.parsing_report['accuracy'] > 0.7:
                    ...
                    print(f"  ✓ Tabla {idx+1} extraída (Lattice, accuracy={table.parsing_report['accuracy']:.2f})")
        ...
        try:
            ...
            for idx, table in enumerate(stream_tables):
                if table.parsing_report['accuracy'] > 0.6:
                    ...
                    print(f"  ✓ Tabla {idx+1} extraída (Stream, accuracy={table.parsing_report['accuracy']:.2f})")
        except Exception as e:
        ...
        for cluster_id in set(clustering.labels_):
            ...
            if len(cluster_indices) > 1:
                ...
                sorted_indices = sorted(
                    cluster_indices,
                    key=lambda i: tables[i].page_number
                )
    ...
    def _consolidate_entities(
        ...
        for cluster_id in set(labels):
            ...
            best_entity = max(cluster_entities, key=lambda e: (
                len(e.name),
                e.specificity_score,
                e.mentioned_count
            ))
    ...
    def _score_entity_specificity(
        ...
        for entity in entities:
            ...
            institutional_words = [
                'secretaría', 'dirección', 'oficina', 'departamento',
                'coordinación', 'gerencia', 'subdirección'
            ]
            inst_score = float(any(
                word in entity.name.lower() for word in institutional_words
            ))
    ...
    def calculate_quality_score(
        ...
        return QualityScore(
            ...
            evidence={
                'n_tables': len(tables),
                'n_financial_indicators': len(financial_analysis.get('financial_indicators', [])),
                'n_responsible_entities': len(responsible_entities),
                'risk_level': financial_analysis.get('risk_assessment', {}).get('risk_score', 0)
            }
        )
    ...
    def _score_financial_dimension(self, financial_analysis: Dict[str, Any]) -> float:
        ...
        diversity = financial_analysis.get('funding_sources', {}).get('diversity_index', 0)
    ...
    def _score_pdet_alignment(self, text: str, tables: List[ExtractedTable]) -> float:
        ...
        pillars_mentioned = sum(
            1 for pillar in self.context.PDET_PILLARS
        ...
        pdet_keywords = ['pdet', 'patr', 'transformación regional', 'paz', 'víctimas', 'reconciliación']
