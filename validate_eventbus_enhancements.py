        ...
        try:
            ...
            for node in ast.walk(tree):
                if (
                    ...
                    and node.name == "ContradictionDetectorV2"
                ):
                            ...
                            for subnode in ast.walk(item):
                                if isinstance(subnode, ast.Call):
                                    if (
                                        ...
                                        and subnode.func.attr == "subscribe"
                                    ):
                                        if subnode.args:
                                            if isinstance(
                                                subnode.args[0], ast.Constant
                                            ):
                                                subscriptions.append(
                                                    subnode.args[0].value
                                                )

                            print(f"   Found subscriptions: {subscriptions}")

                            if (
                                "graph.edge_added" in subscriptions
                                and "graph.node_added" in subscriptions
                            ):
                                print(
                                    "   ✓ Both graph.edge_added and graph.node_added subscriptions found"
                                )
                                return True
                            else:
    ...
    def validate_circuit_breaker(self) -> bool:
        ...
        try:
            ...
            if len(found_components) == len(required_components):
                ...
                print(
                    f"   ✗ Missing {len(required_components) - len(found_components)} components"
                )
                return False
