...
def mock_extraction_pipeline():
    ...
    async def extract_complete(pdf_path):
        return ExtractionResult(
            ...
            tables=[{"title": "Table 1", "headers": ["A", "B"], "rows": [[1, 2]]}],
...
@pytest.mark.asyncio
async def test_unified_pipeline_execution(
    orchestrator,
    mock_extraction_pipeline,
    mock_causal_builder,
    mock_bayesian_engine,
    mock_validator,
):
...
async def test_prior_snapshot_immutability(orchestrator):
    ...
    # Modify prior store
    orchestrator.prior_store.update_mechanism_prior("tecnico", 5.0, "test update")
    ...
    assert not (
        snapshot1.priors["tecnico"] == pytest.approx(5.0, rel=1e-9, abs=1e-12)
    )  # replaced float inequality with negated pytest.approx


@pytest.mark.asyncio
async def test_circular_dependency_resolution(
    orchestrator,
    mock_extraction_pipeline,
    mock_causal_builder,
    mock_bayesian_engine,
...
@pytest.mark.asyncio
async def test_bayesian_convergence_tracking(
    orchestrator, mock_extraction_pipeline, mock_causal_builder, mock_bayesian_engine
):
...
async def test_scoring_consistency(
    ...
    # Create mock scorer
    mock_scorer = Mock()
    mock_scorer.calculate_all_levels = Mock(
        return_value={
            "micro": {
                f"P{i}-D{j}-Q{k}": 0.7
...
@pytest.mark.asyncio
async def test_harmonic_front_4_penalty_learning(
    orchestrator, mock_extraction_pipeline, mock_causal_builder, mock_bayesian_engine
):
...
@pytest.mark.asyncio
async def test_deterministic_fixture_reproducibility(
    orchestrator, mock_extraction_pipeline, mock_causal_builder, mock_bayesian_engine
):
...
@pytest.mark.asyncio
async def test_async_profiling_bottleneck_identification(
    orchestrator, mock_extraction_pipeline
):
...
def test_prior_snapshot_correctness():
    ...
    assert latest["priors"]["tecnico"]["alpha"] == pytest.approx(
        3.5, rel=1e-9, abs=1e-12
