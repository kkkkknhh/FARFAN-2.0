...
class TestAxiomaticValidatorIntegration(unittest.TestCase):
    ...
    def test_validation_execution_order(self):
        ...
        self.assertEqual(execution_order, ['structural', 'semantic', 'regulatory'],
    ...
    def test_teoria_cambio_score_mappings(self):
        ...
        structural_result.caminos_completos = [['I', 'P', 'Pr', 'R', 'C']]  # 1 complete path
    ...
    def test_dnp_validator_score_mappings(self):
        ...
        dnp_result.indicadores_mga_usados = ['EDU-001', 'EDU-002', 'SAL-001', 'INF-001', 'AMB-001']
        ...
        self.assertGreater(d1_q5, 0.7, "D1-Q5 should be >0.7 with competency compliance")
        self.assertGreater(d4_q5, 0.7, "D4-Q5 should be >0.7 with MGA compliance and 5 indicators")
        
        print(f"✓ Test 4 PASSED: DNP → D1-Q5={d1_q5:.3f}, D4-Q5={d4_q5:.3f} (BPIN: 5 indicators)")
    ...
    def test_governance_trigger_1_contradiction_density(self):
        ...
        validator._validate_structural = lambda g: Mock(
            es_valida=True, violaciones_orden=[], categorias_faltantes=[],
            caminos_completos=[[]], sugerencias=[]
        )
        validator._validate_semantic = lambda g, c: contradictions
        validator._validate_regulatory = lambda c, f: Mock(
            score_total=70.0, cumple_competencias=True, cumple_mga=True,
        ...
        self.assertGreater(result.contradiction_density, 0.05, 
                          "Contradiction density should exceed threshold")
        self.assertTrue(result.requires_manual_review, 
                       "Manual review should be required")
        self.assertEqual(result.hold_reason, 'HIGH_CONTRADICTION_DENSITY',
                        "Hold reason should be HIGH_CONTRADICTION_DENSITY")
        
        print(f"✓ Test 5a PASSED: Governance Trigger 1 activated (density={result.contradiction_density:.4f} > 0.05)")
    ...
    def test_governance_trigger_2_d6_threshold(self):
        ...
        structural_result = Mock()
        structural_result.es_valida = False
        structural_result.violaciones_orden = [('A', 'B'), ('C', 'D'), ('E', 'F')]  # 3 violations
        structural_result.categorias_faltantes = ['INSUMOS', 'CAUSALIDAD']  # Missing 2 categories
        ...
        self.assertTrue(d6_q1 < 0.55 or d6_q2 < 0.55,
                       f"At least one D6 score should be <0.55 (D6-Q1={d6_q1:.3f}, D6-Q2={d6_q2:.3f})")
        self.assertTrue(result.requires_manual_review,
                       "Manual review should be required for low D6 scores")
        self.assertEqual(result.hold_reason, 'D6_SCORE_BELOW_THRESHOLD',
                        "Hold reason should be D6_SCORE_BELOW_THRESHOLD")
        
        print(f"✓ Test 5b PASSED: Governance Trigger 2 activated (D6-Q1={d6_q1:.3f}, D6-Q2={d6_q2:.3f} < 0.55)")
    ...
    def test_governance_trigger_3_structural_penalties(self):
        ...
        chunks = [self.SemanticChunk(text="Test", dimension="ESTRATEGICO")]
        result = validator.validate_complete(mock_graph, chunks, prior_builder=mock_prior_builder)
        
        # Verify penalty was applied
        self.assertLess(result.structural_penalty_factor, 1.0,
                       "Penalty factor should be <1.0 with violations")
        
        # Verify penalty matches expected values for 2 violations
        self.assertAlmostEqual(result.structural_penalty_factor, 0.8, places=2,
                              msg="2 violations should result in 0.8 penalty factor")
        
        print(f"✓ Test 5c PASSED: Governance Trigger 3 activated (penalty_factor={result.structural_penalty_factor:.2f} for 2 violations)")
    ...
    def test_result_aggregation(self):
        ...
        self.assertEqual(result.contradiction_density, 0.05, "Should aggregate contradiction density")
        self.assertEqual(result.regulatory_score, 85.0, "Should aggregate regulatory score")
        ...
        self.assertEqual(len(result.bpin_indicators), 2, "Should aggregate BPIN indicators")
        
        print("✓ Test 7 PASSED: Result aggregation verified (6 score mappings, 2 BPIN indicators)")
