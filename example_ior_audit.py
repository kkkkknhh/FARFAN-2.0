...
def demonstrate_ior_audit():
    ...
    if high_confidence:
        ...
        logger.info(
            f"  Confidence {confidence_score:.1f}% >= {high_confidence_threshold}% threshold"
        )
        logger.info(f"  Complies with Colombian DNP 2023 audit standards")
    else:
        logger.warning(
            f"⚠ Low confidence: {confidence_score:.1f}% < {high_confidence_threshold}%"
        )
    ...
    if math.isclose(overall_compliance, 100.0, rel_tol=1e-9, abs_tol=1e-12):  # replaced float equality with isclose (tolerance from DEFAULT_FLOAT_TOLS)
