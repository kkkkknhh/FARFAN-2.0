...
class ExtractedTable(BaseModel):
    ...
    def validate_data_structure(cls, v):
        ...
        """
        if not v:
            raise ValueError("Table data cannot be empty - Hoop Test failed (Audit Point 1.3)")
        if not all(isinstance(row, list) for row in v):
            raise ValueError("All rows must be lists - Schema integrity violation (Audit Point 1.3)")
        return v
    ...
    def validate_bpin_if_required(cls, v, values):
        ...
        if table_type in ['financial', 'investment', 'budget'] and not v:
            ...
            logging.warning(f"BPIN missing for {table_type} table - DNP compliance degraded")
        return v
...
class SemanticChunk(BaseModel):
    ...
    def validate_chunk_id(cls, v):
        ...
        """
        if not v:
            raise ValueError("chunk_id is required for provenance traceability (Audit Point 1.2)")
        # Validate SHA-256 format (64 hex characters)
        if not (len(v) == 64 and all(c in '0123456789abcdef' for c in v.lower())):
            raise ValueError(f"chunk_id must be valid SHA-256 hash (64 hex chars), got: {v[:20]}...")
        return v
...
class ExtractionPipeline:
        ...
        while start < text_length:
            ...
            if chunk_text:
                ...
                try:
                    ...
                    self.logger.error(f"Chunk validation failed (Audit 1.3 Hoop Test): {e}")
    ...
    def _compute_sha256(self, pdf_path: str) -> str:
        ...
        self.logger.info(f"Computed source_pdf_hash: {hex_digest[:16]}... (Audit Point 1.2)")
        return hex_digest
