...
def test_get_component_required(deterministic_orchestrator):
    ...
    with pytest.raises(DependencyNotInjectedError, match="Required component 'teoria_cambio' not injected"):
...
def test_get_component_optional(deterministic_orchestrator):
    ...
    component = deterministic_orchestrator._get_component('teoria_cambio', required=False)
...
def test_get_component_success(deterministic_orchestrator):
    ...
    component = deterministic_orchestrator._get_component('teoria_cambio', required=True)
...
def test_validate_contract_inputs_success(deterministic_orchestrator):
    ...
    deterministic_orchestrator._validate_contract_inputs(
        PipelinePhase.PHASE_1_EXTRACTION,
        context
    )
...
def test_emit_telemetry(deterministic_orchestrator):
    ...
    assert 'telemetry.start.PHASE_1_EXTRACTION' in deterministic_orchestrator.metrics.metrics
...
@pytest.mark.asyncio
async def test_execute_phase_1_extraction_requires_component(deterministic_orchestrator):
...
@pytest.mark.asyncio
async def test_full_pipeline_execution_with_mocks(deterministic_orchestrator, mock_components):
...
@pytest.mark.asyncio
async def test_pipeline_execution_determinism(deterministic_orchestrator, mock_components):
...
@pytest.mark.asyncio
async def test_pipeline_handles_phase_error(deterministic_orchestrator, mock_components):
...
@pytest.mark.asyncio
async def test_pipeline_logs_audit_on_error(deterministic_orchestrator, mock_components):
