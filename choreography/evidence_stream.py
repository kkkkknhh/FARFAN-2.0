...
class StreamingBayesianUpdater:
    ...
    async def update_from_stream(
        ...
        logger.info(
            f"Starting streaming Bayesian update for '{prior.mechanism_name}'"
        )
        ...
        async for chunk in evidence_stream:
            ...
            if await self._is_relevant(chunk, prior.mechanism_name):
                ...
                # Bayesian update
                current_posterior = self._bayesian_update(
                    current_posterior,
                    likelihood
                )
    ...
    async def _is_relevant(self, chunk: SemanticChunk, mechanism_name: str) -> bool:
        ...
        if is_relevant:
            logger.debug(f"Chunk {chunk.get('chunk_id')} is relevant to {mechanism_name}")
