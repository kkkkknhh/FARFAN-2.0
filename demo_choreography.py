...
from choreography.event_bus import EventBus, PDMEvent, ContradictionDetectorV2
...
async def main():
    ...
    async def on_contradiction(event: PDMEvent):
        ...
        print(f"  ⚠️  CONTRADICTION ({severity}): {edge.get('source')} -> {edge.get('target')}")
    ...
    await bus.publish(PDMEvent(
        ...
        run_id='demo_run',
        payload={'source': 'Objetivo_A', 'target': 'Resultado_B', 'relation': 'leads_to'}
    ))
    ...
    await bus.publish(PDMEvent(
        event_type='graph.edge_added',
        run_id='demo_run',
        payload={'source': 'Programa_X', 'target': 'Programa_X', 'relation': 'depends_on'}
    ))
    await asyncio.sleep(0.1)
    ...
    print(f"  Credible Interval (95%): [{posterior.credible_interval_95[0]:.3f}, {posterior.credible_interval_95[1]:.3f}]")
