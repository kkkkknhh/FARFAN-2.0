...
class TestDimensionWeights(unittest.TestCase):
    ...
    def test_all_policies_have_six_dimensions(self):
        for policy in [f"P{i}" for i in range(1, 11)]:
            ...
            self.assertEqual(
                set(DIMENSION_WEIGHTS[policy].keys()),
                {"D1", "D2", "D3", "D4", "D5", "D6"},
            )
    ...
    def test_dimension_weights_sum_to_one(self):
        for policy, weights in DIMENSION_WEIGHTS.items():
            ...
            self.assertAlmostEqual(
                total, 1.0, places=9, msg=f"{policy} weights sum to {total}, not 1.0"
            )
    ...
    def test_all_weights_positive(self):
        for policy, weights in DIMENSION_WEIGHTS.items():
            for dimension, weight in weights.items():
                self.assertGreater(
                    weight, 0.0, msg=f"{policy}-{dimension} has non-positive weight"
                )
                self.assertLessEqual(
                    weight, 1.0, msg=f"{policy}-{dimension} weight exceeds 1.0"
                )
...
class TestRubricThresholds(unittest.TestCase):
    ...
    def test_threshold_ranges_valid(self):
        for category, (low, high) in RUBRIC_THRESHOLDS.items():
            self.assertGreaterEqual(low, 0.0, msg=f"{category} lower bound < 0")
            self.assertLessEqual(high, 1.0, msg=f"{category} upper bound > 1.0")
            self.assertLess(
                low, high, msg=f"{category} has inverted range [{low}, {high}]"
            )
...
class TestCanonicalNotationValidator(unittest.TestCase):
    ...
    def test_question_format_validation(self):
        ...
        # Valid formats
        valid_ids = ["P1-D1-Q1", "P5-D3-Q2", "P10-D6-Q5"]
...
class TestScoringAggregation(unittest.TestCase):
    ...
    def test_dimension_score_calculation(self):
        ...
        question_scores = [
            QuestionScore(f"P1-D1-Q{i}", 0.8, "bueno") for i in range(1, 6)
        ]
    ...
    def test_policy_score_calculation(self):
        ...
        for dim_num in range(1, 7):
            ...
            question_scores = [
                QuestionScore(f"P2-{dimension}-Q{i}", 0.7, "bueno") for i in range(1, 6)
            ]
            dim_score = self.engine.calculate_dimension_score(
                "P2", dimension, question_scores
            )
    ...
    def test_weighted_policy_score(self):
        ...
        expected = sum(
            test_scores[dim] * DIMENSION_WEIGHTS["P3"][dim]
    ...
    def test_cluster_score_calculation(self):
        ...
        for policy in ["P1", "P2", "P8"]:
            ...
            for dim_num in range(1, 7):
                ...
                dim_score = self.engine.calculate_dimension_score(
                    policy, dimension, question_scores
                )
                dimension_scores.append(dim_score)
        ...
        cluster_score = self.engine.calculate_cluster_score(
            "derechos_humanos", policy_scores
        )

        self.assertEqual(cluster_score.cluster_name, "derechos_humanos")
        self.assertEqual(len(cluster_score.policy_scores), 3)
        self.assertAlmostEqual(
            cluster_score.weight, CLUSTER_WEIGHTS["derechos_humanos"], places=5
        )
