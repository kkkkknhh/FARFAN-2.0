...
class TestBayesianPriorBuilder(unittest.TestCase):
    ...
    def test_build_mechanism_prior(self):
        ...
        mechanism_evidence = MechanismEvidence(
            type="técnico", verb_sequence=["implementar", "ejecutar", "evaluar"]
        )

        context = ColombianMunicipalContext(overall_pdm_embedding=self.context_emb)
    ...
    def test_mechanism_type_coherence(self):
        ...
        coherence = self.builder._validate_mechanism_type_coherence(
            verb_sequence, "producto", "resultado"
        )
...
class TestBayesianSamplingEngine(unittest.TestCase):
    ...
    def test_sample_mechanism_posterior_with_evidence(self):
        ...
        evidence = [
            EvidenceChunk(
                chunk_id="chunk1", text="Test chunk 1", cosine_similarity=0.85
            ),
            EvidenceChunk(
                chunk_id="chunk2", text="Test chunk 2", cosine_similarity=0.75
            ),
            EvidenceChunk(
                chunk_id="chunk3", text="Test chunk 3", cosine_similarity=0.65
            ),
        ]
    ...
    def test_reproducibility(self):
        ...
        evidence = [EvidenceChunk("c1", "text", 0.8), EvidenceChunk("c2", "text", 0.7)]
        ...
        self.assertAlmostEqual(
            posterior1.posterior_mean, posterior2.posterior_mean, delta=0.01
        )
