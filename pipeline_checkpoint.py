...
class CheckpointIndex:
    ...
    def from_dict(cls, data: Dict) -> 'CheckpointIndex':
        """Create from dictionary"""
        checkpoints = {k: CheckpointMetadata.from_dict(v) 
...
class PipelineCheckpoint:
    ...
    def __init__(
        ...
        logger.info(f"Encryption: {self.enable_encryption}, Incremental: {self.enable_incremental}")
        logger.info(f"Retention: {self.retention_policy.value} (count={self.retention_count}, age={self.retention_age_days}d)")
    ...
    def save(
        ...
        if self.enable_incremental and not force_full and self.index.last_full_checkpoint_id:
            ...
            try:
                ...
                if delta_size < full_size * 0.7:  # 70% threshold
                    state_to_save = delta
                    is_incremental = True
                    logger.info(f"Creating incremental checkpoint (delta size: {delta_size}, full size: {full_size})")
                else:
                    logger.info(f"Delta not beneficial, creating full checkpoint")
            except Exception as e:
                logger.warning(f"Failed to create incremental checkpoint: {e}, creating full checkpoint")
        ...
        logger.info(f"Checkpoint saved: {checkpoint_id} ({metadata.size_bytes} bytes, hash: {hash_sha256[:16]}...)")
    ...
    def load(self, checkpoint_id: str, verify_hash: bool = True) -> Dict[str, Any]:
        ...
        if metadata.is_encrypted:
            if self.cipher is None:
                raise RuntimeError("Checkpoint is encrypted but encryption is not enabled")
        ...
        if metadata.is_incremental:
            if metadata.base_checkpoint_id is None:
                raise ValueError(f"Incremental checkpoint {checkpoint_id} missing base_checkpoint_id")
            logger.info(f"Reconstructing from base checkpoint: {metadata.base_checkpoint_id}")
    ...
    def delete_checkpoint(self, checkpoint_id: str):
        ...
        if self.index.last_full_checkpoint_id == checkpoint_id:
            ...
            full_checkpoints = [
                c for c in self.index.checkpoints.values() 
                if not c.is_incremental
            ]
            if full_checkpoints:
    ...
    def configure_cloud_backup(self, config: CloudBackupConfig):
        ...
        logger.info(f"Cloud backup configured: {config.provider.value} (enabled: {config.enabled})")
