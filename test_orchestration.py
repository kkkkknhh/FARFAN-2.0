...
def test_feedback_extraction():
    ...
    quality_score = QualityScore(
        overall_score=0.7,
        dimension_scores={'D1': 0.7, 'D2': 0.6, 'D3': 0.8, 'D4': 0.7, 'D5': 0.65, 'D6': 0.75}
    )
...
def test_prior_updates():
    ...
    assert abs(updated_alpha - initial_alpha * 0.9) < 0.01  # Allow small floating point error
...
async def test_async_analyze_plan():
    ...
    try:
        ...
        assert orchestrator.state in [PDMAnalysisState.COMPLETED, PDMAnalysisState.FAILED]
