...
POLICY_TO_CLUSTER = {p: cluster for cluster, policies in CLUSTER_TO_POLICIES.items() for p in policies}
...
class ScoringSystemAuditor:
    ...
    def audit_matrix_structure(self, question_responses: Dict[str, Any]) -> None:
        ...
        if self.report.total_questions_found != EXPECTED_TOTAL_QUESTIONS:
            self.report.micro_issues.append(AuditIssue(
                ...
                expected=EXPECTED_TOTAL_QUESTIONS, actual=self.report.total_questions_found,
        ...
        if missing:
            self.report.micro_issues.append(AuditIssue(
                ...
                expected=sorted(expected_policies), actual=sorted(self.report.policies_found),
        ...
        if missing_dim:
            self.report.micro_issues.append(AuditIssue(
                ...
                expected=sorted(expected_dimensions), actual=sorted(self.report.dimensions_found),
                recommendation=f"Add missing: {sorted(missing_dim)}"
            ))
            self.report.matrix_valid = False
        if not self.report.matrix_valid:
            self.report.matrix_valid = len([i for i in self.report.micro_issues if i.category == "matrix_structure"]) == 0
        else:
    ...
    def audit_rubric_thresholds(self, question_responses: Dict[str, Any]) -> None:
        ...
        for question_id, response in question_responses.items():
            ...
            band_counts[band] += 1
        logger.info(f"  EXCELENTE (≥{THRESHOLD_EXCELENTE}): {band_counts[QualityBand.EXCELENTE]}")
        logger.info(f"  BUENO ({THRESHOLD_BUENO}-{THRESHOLD_EXCELENTE}): {band_counts[QualityBand.BUENO]}")
        logger.info(f"  ACEPTABLE ({THRESHOLD_ACEPTABLE}-{THRESHOLD_BUENO}): {band_counts[QualityBand.ACEPTABLE]}")
        logger.info(f"  INSUFICIENTE (<{THRESHOLD_ACEPTABLE}): {band_counts[QualityBand.INSUFICIENTE]}")
        if band_counts[QualityBand.INSUFICIENTE] > EXPECTED_TOTAL_QUESTIONS * 0.3:
            self.report.rubric_issues.append(AuditIssue(
                ...
                description="High proportion of insufficient scores", location="MICRO level",
            ...
            if '-D6-' in question_id and hasattr(response, 'nota_cuantitativa'):
                ...
                if score < THRESHOLD_D6_CRITICAL:
                    self.report.d6_scores_below_threshold.append({
                        ...
                        'threshold': THRESHOLD_D6_CRITICAL, 'gap': THRESHOLD_D6_CRITICAL - score
                    })
        if self.report.d6_scores_below_threshold:
            logger.warning(f"⚠ {len(self.report.d6_scores_below_threshold)} D6 scores below {THRESHOLD_D6_CRITICAL}")
            self.report.rubric_issues.append(AuditIssue(
                category="d6_theory_of_change", severity="CRITICAL",
                description=f"D6 scores below threshold ({THRESHOLD_D6_CRITICAL})", location="D6",
    ...
    def audit_dnp_integration(self, dnp_results: Any, macro_report: Optional[Dict[str, Any]]) -> None:
        logger.info("Auditing DNP integration...")
        required = ['cumple_competencias', 'cumple_mga', 'nivel_cumplimiento', 'score_total']
        for attr in required:
            if not hasattr(dnp_results, attr):
                self.report.dnp_issues.append(AuditIssue(
                    ...
                    expected=attr, actual="missing", recommendation=f"Add {attr} to DNP validator"
                ))
...
if __name__ == "__main__":
    ...
    print(f"Expected matrix: {EXPECTED_POLICIES}×{EXPECTED_DIMENSIONS}×{EXPECTED_QUESTIONS_PER_DIM} = {EXPECTED_TOTAL_QUESTIONS}")
