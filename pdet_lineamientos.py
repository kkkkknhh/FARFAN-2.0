...
class LineamientosPDET:
    ...
    def _initialize_lineamientos(self):
        ...
        self.add_lineamiento(
            LineamientoPDET(
                ...
                enfoque_requerido=[
                    EnfoquePDET.TERRITORIAL,
                    EnfoquePDET.DIFERENCIAL,
                    EnfoquePDET.REPARADOR,
                ],
        ...
        self.add_lineamiento(
            LineamientoPDET(
                ...
                articulacion_institucional=[
                    ...
                    "PAP - Planes Departamentales de Agua",
                ],
    ...
    def validar_cumplimiento_pdet(
        ...
        if (
            participacion.get("comunitaria", 0)
            < self.requisitos.participacion_comunitaria_minima
        ):
        ...
        porcentaje_rural = (
            (presupuesto_rural / presupuesto_total * 100)
    ...
    def generar_matriz_priorizacion_pdet(self) -> Dict[PilarPDET, List[str]]:
        ...
        for pilar in PilarPDET:
            ...
            matriz[pilar] = [
                f"{lin.codigo}: {lin.titulo}" for lin in lineamientos_pilar
            ]
        return matriz

    def recomendar_lineamientos(
        self, sector: str, poblacion_victimas: bool = False
    ) -> List[LineamientoPDET]:
        ...
        if "educaci" in sector_lower:
            ...
            "agua" in sector_lower
            or "saneamiento" in sector_lower
            or "vivienda" in sector_lower
        ):
            recomendaciones.extend(
                self.get_by_pilar(PilarPDET.VIVIENDA_AGUA_SANEAMIENTO)
            )
        elif (
            "v" in sector_lower
            or "transporte" in sector_lower
            or "conectividad" in sector_lower
        ):
            recomendaciones.extend(
                self.get_by_pilar(PilarPDET.INFRAESTRUCTURA_CONECTIVIDAD)
            )
        elif (
            "agr" in sector_lower
            or "producc" in sector_lower
            or "econom" in sector_lower
        ):
        ...
        if poblacion_victimas:
            recomendaciones.extend(
                self.get_by_pilar(PilarPDET.RECONCILIACION_CONVIVENCIA)
            )
...
if __name__ == "__main__":
    ...
    # Example validation
    participacion = {"comunitaria": 75.0, "victimas": 45.0, "mujeres": 35.0}
