...
def demonstrate_audit_point_23():
    ...
    print(f"PDET Alignment: {pdet_alignment:.2f}")
    print(
        f"Alignment Penalty: {'Applied (1.2×)' if result['alignment_penalty_applied'] else 'Not Applied'}"
    )
    print(
    ...
    if result["alignment_causing_failure"]:
        print(
            f"⚠️  WARNING: Low alignment degraded quality from EXCELENTE to {result['d5_q4_quality'].upper()}"
        )
    ...
    if result["alignment_causing_failure"]:
        ...
        print(
            f"   With penalty: Risk={result['risk_score']:.3f} → {result['d5_q4_quality'].upper()}"
        )
