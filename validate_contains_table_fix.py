...
def validate_contains_table_fix():
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Call):
            if (
                ...
                and node.func.attr == "_contains_table"
            ):
    ...
    if actual_params != expected_params:
        print(
            f"✗ Definition parameters don't match. Expected {expected_params}, got {actual_params}"
        )
        return False
    ...
    if arg_names != ["chunk_start", "chunk_end", "tables"]:
        print(
            f"⚠ Warning: Argument names are {arg_names}, expected ['chunk_start', 'chunk_end', 'tables']"
        )
    else:
