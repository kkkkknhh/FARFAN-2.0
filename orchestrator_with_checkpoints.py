    ...
    def __init__(
        ...
        if cloud_backup:
            ...
            cloud_config = CloudBackupConfig(
                enabled=True,
                provider=provider_map.get(cloud_provider, CloudBackupProvider.LOCAL_ONLY),
    ...
    def process_plan_with_checkpoints(
        ...
        try:
            ...
            if resume_stage < 8:
                ...
                self._save_checkpoint(ctx, stage=8, policy_code=policy_code, force_full=True)
            ...
            logger.info("💾 State has been checkpointed - can resume with resume_from parameter")
            raise
    
    def _save_checkpoint(self, ctx, stage: int, policy_code: str, force_full: bool = False):
...
def main():
    ...
    parser.add_argument("--cloud-backup", action="store_true", help="Enable cloud backup")
    parser.add_argument("--cloud-provider", default="aws_s3", 
    ...
    parser.add_argument("--checkpoint-stats", action="store_true",
    ...
    try:
        ...
        print(f"Checkpoints saved to: {args.checkpoint_dir or args.output_dir + '/checkpoints'}")
