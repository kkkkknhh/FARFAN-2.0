...
class BayesianCausalInference:
    ...
    def calculate_posterior(
        ...
        beta_post = (
            self.prior_beta + (1 - evidence_strength) * observations * domain_weight
        )
                ...
                if i != j:
                    ...
                    if causal_strength > 0.3:
                        self.causal_network.add_edge(
                            f"stmt_{i}", f"stmt_{j}", weight=causal_strength
                        )

    def _estimate_causal_strength(
        self, cause: PolicyStatement, effect: PolicyStatement
    ) -> float:
        ...
        for marker, weight in causal_markers.items():
            if marker in cause.text.lower():
                if any(
                    ...
                    or any(
                        synonym in effect_text for synonym in entity.get("synonyms", [])
                    )
...
class TemporalLogicVerifier:
                    ...
                    conflicts.append(
                        {
                            ...
                            "severity": self._calculate_temporal_severity(
                                conflict_type
                            ),
                        }
                    )
    ...
    def _intervals_overlap(
        self, interval_a: Tuple[float, float], interval_b: Tuple[float, float]
    ) -> bool:
    ...
    def _are_mutually_exclusive(
        self, stmt_a: PolicyStatement, stmt_b: PolicyStatement
    ) -> bool:
    ...
    def _requires_precedence(
        ...
        for indicator in precedence_indicators:
            if indicator in stmt_a.text.lower():
                if any(entity.get("text") in stmt_b.text for entity in stmt_a.entities):
                    return True
    ...
    def _contradicts_always(
        self, stmt: PolicyStatement, always_stmt: PolicyStatement
    ) -> bool:
        """Verifica contradicción con restricción 'always'"""
        negation_patterns = ["no", "nunca", NINGUN_LITERAL, "sin", "excepto", "salvo"]

        has_negation = any(
            pattern in stmt.text.lower() for pattern in negation_patterns
        )
    ...
    def _calculate_temporal_consistency_score(
        self, num_conflicts: int, num_events: int
    ) -> float:
...
class GraphNeuralReasoningEngine:
    ...
    def detect_implicit_contradictions(
        self, statements: List[PolicyStatement], knowledge_graph: nx.DiGraph
        ...
        for i, stmt_a in enumerate(statements):
            for j, stmt_b in enumerate(statements[i + 1 :], start=i + 1):
                contradiction_score = self._compute_contradiction_score(
                    node_embeddings[i], node_embeddings[j], statements[i], statements[j]
                )
    ...
    def _build_geometric_graph(
        self, statements: List[PolicyStatement], nx_graph: nx.DiGraph
    ) -> Data:
    ...
    def _compute_contradiction_score(
        ...
        has_negation_a = any(
            neg in stmt_a.text.lower() for neg in ["no", "nunca", NINGUN_LITERAL]
        )
        has_negation_b = any(
            neg in stmt_b.text.lower() for neg in ["no", "nunca", NINGUN_LITERAL]
        )
        ...
        contradiction_score = (
            (1 - cosine_sim) * 0.5 + negation_factor * 0.3 + entity_overlap * 0.2
        )

        return min(1.0, contradiction_score)

    def _extract_path_attention(
        self, node_i: int, node_j: int, *attention_layers
    ) -> np.ndarray:
...
class AdvancedStatisticalTesting:
    ...
    def resource_allocation_chi_square(
        ...
        chi2_stat = np.sum(
            (observed - expected_matrix) ** 2 / (expected_matrix + 1e-10)
        )
...
class PolicyContradictionDetectorV2:
    ...
    def detect(
        ...
        logger.info(
            f"Detectadas {len(semantic_contradictions)} contradicciones semánticas"
        )

        numerical_contradictions = self._detect_numerical_inconsistencies_robust(
            statements
        )
        contradictions.extend(numerical_contradictions)
        logger.info(
            f"Detectadas {len(numerical_contradictions)} inconsistencias numéricas"
        )
        ...
        logger.info(
            f"Detectadas {len(gnn_contradictions)} contradicciones implícitas (GNN)"
        )
        ...
        total_contradictions_for_grading = self._audit_metrics.get(
            "total_contradictions", len(contradictions)
        )
        quality_grade_classification = (
            "Excelente"
            if total_contradictions_for_grading < 5
            else "Bueno"
            if total_contradictions_for_grading < 10
            else "Regular"
        )

        audit_summary = {
            "total_contradictions": self._audit_metrics.get(
                "total_contradictions", len(contradictions)
            ),
        ...
        regulatory_analysis = self._analyze_regulatory_constraints(
            statements, text, temporal_conflicts
        )
        ...
        return {
            ...
            "contradictions": [
                self._serialize_contradiction(c) for c in contradictions
            ],
            "total_contradictions": len(contradictions),
            "critical_severity_count": sum(
                1 for c in contradictions if c.severity > 0.85
            ),
            "high_severity_count": sum(
                1 for c in contradictions if 0.7 < c.severity <= 0.85
            ),
            "medium_severity_count": sum(
                1 for c in contradictions if 0.5 < c.severity <= 0.7
            ),
    ...
    def _extract_policy_statements(
        self, text: str, dimension: PolicyDimension
        ...
        for sent in doc.sents:
            ...
            quantitative_claims = self._extract_structured_quantitative_claims(
                sent.text
            )
            ...
            statement = PolicyStatement(
                ...
                context_window=self._get_extended_context(
    ...
    def _generate_contextual_embeddings(
        ...
        contextual_embeddings = self.semantic_model.encode(
            contextual_texts, convert_to_numpy=True, show_progress_bar=False
        )

        for stmt, std_emb, ctx_emb in zip(
            statements, standard_embeddings, contextual_embeddings
        ):
            ...
            for stmt_b in statements[i + 1 :]:
                ...
                if (
                    ...
                    evidence = ContradictionEvidence(
                        ...
                        severity=self._calculate_comprehensive_severity(
                            stmt_a, stmt_b, nli_result["score"]
                        ),
                        ...
                        resolution_suggestions=self._generate_resolution_strategies(
                            ContradictionType.SEMANTIC_OPPOSITION, stmt_a, stmt_b
                        ),
                    )
    ...
    def _collect_comparable_claim_pairs(
        ...
        Tuple[
            Tuple[PolicyStatement, Dict[str, Any]],
            Tuple[PolicyStatement, Dict[str, Any]],
        ]
    ]:
                    ...
                    for claim_a in stmt_a.quantitative_claims:
                        for claim_b in stmt_b.quantitative_claims:
                            if self._are_comparable_claims_advanced(claim_a, claim_b):
                                claim_pairs.append(
                                    ((stmt_a, claim_a), (stmt_b, claim_b))
                                )

        return claim_pairs

    def _validate_statistical_divergence(
        self, divergence: float, p_value: float
    ) -> bool:
    ...
    def _detect_numerical_inconsistencies_robust(
        ...
        divergences, adjusted_p_values = (
            self.statistical_tester.numerical_divergence_test(test_data)
        )
    ...
    def _detect_temporal_conflicts_formal(
        ...
        _, conflicts, _ = self.temporal_verifier.verify_temporal_consistency(
            temporal_statements
        )
        ...
        for conflict in conflicts:
            ...
            evidence = ContradictionEvidence(
                ...
                semantic_similarity=self._calculate_contextual_similarity(
                    stmt_a, stmt_b
                ),
    ...
    def _detect_implicit_contradictions_gnn(
        ...
        for stmt_a, stmt_b, score, attention_weights in gnn_results:
            confidence, _ = self.bayesian_engine.calculate_posterior(
                ...
                observations=(
                    len(attention_weights) if len(attention_weights) > 0 else 1
                ),
                domain_weight=1.1,
                prior_knowledge=0.5,
            )
            ...
            evidence = ContradictionEvidence(
                ...
                affected_dimensions=[stmt_a.dimension, stmt_b.dimension],
                resolution_suggestions=self._generate_resolution_strategies(
                    ContradictionType.LOGICAL_INCOMPATIBILITY, stmt_a, stmt_b
                ),
    ...
    def _detect_causal_inconsistencies(
        ...
        for i, stmt_a in enumerate(statements):
            for j, stmt_b in enumerate(statements[i + 1 :], start=i + 1):
                ...
                if causal_network.has_edge(node_a, node_b) and causal_network.has_edge(
                    node_b, node_a
                ):
                    ...
                    if abs(weight_ab - weight_ba) < 0.3:
                        ...
                        evidence = ContradictionEvidence(
                            ...
                            resolution_suggestions=self._generate_resolution_strategies(
                                ...
                                stmt_b,
                                context={
                                    "conflict_type": "circular_causality",
                                    "weight_ab": weight_ab,
                                    "weight_ba": weight_ba,
                                },
                            ),
                ...
                if has_weak_causal_link and gnn_score > 0.65:
                    ...
                    evidence = ContradictionEvidence(
                        ...
                        resolution_suggestions=self._generate_resolution_strategies(
                            ...
                            stmt_b,
                            context={
                                "conflict_type": "structural_incoherence_gnn",
                                "gnn_score": gnn_score,
                            },
                        ),
                        causal_conflict=True,
    ...
    def _check_temporal_resource_overlap(
        ...
        intervals_a = [
            self.temporal_verifier._parse_temporal_interval(m)
    ...
    def _detect_resource_conflicts_statistical(
        ...
        for resource_type, allocations in resource_allocations.items():
            if len(allocations) > 1:
                ...
                chi2_stat, p_value = (
                    self.statistical_tester.resource_allocation_chi_square(
        ...
        for i, stmt in enumerate(statements):
            ...
            for j, other in enumerate(statements):
                if i != j:
                    ...
                    if similarity > 0.65:
                        relation_type = self._determine_relation_type_advanced(
                            stmt, other
                        )
                        edge_weight = similarity * self._get_relation_weight(
                            relation_type
                        )
    ...
    def _compute_individual_coherence_scores(
        ...
        semantic_coherence = self._calculate_global_semantic_coherence_advanced(
            statements
        )
    ...
    def _compute_auxiliary_metrics(
        ...
        confidence_interval = self._calculate_bootstrap_confidence_interval(
            coherence_score, num_statements, n_bootstrap=1000
        )
    ...
    def _calculate_advanced_coherence_metrics(
        ...
        contradiction_entropy, syntactic_complexity, confidence_interval = (
            self._compute_auxiliary_metrics(
                contradictions, text, coherence_score, len(statements)
            )
        )
    ...
    def _calculate_contextual_similarity(
        ...
        if (
            stmt_a.contextual_embedding is not None
            and stmt_b.contextual_embedding is not None
        ):
            return float(
                1
                - np.dot(stmt_a.contextual_embedding, stmt_b.contextual_embedding)
                / (
                    np.linalg.norm(stmt_a.contextual_embedding)
                    * np.linalg.norm(stmt_b.contextual_embedding)
                    + 1e-10
                )
            )
            ...
            return float(
                1
                - np.dot(stmt_a.embedding, stmt_b.embedding)
                / (
                    np.linalg.norm(stmt_a.embedding) * np.linalg.norm(stmt_b.embedding)
                    + 1e-10
                )
            )
        return 0.0
    ...
    def _extract_structured_quantitative_claims(
        self, text: str
    ) -> List[Dict[str, Any]]:
        ...
        patterns = [
            ...
            (
                r"(\d+(?:[.,]\d+)?)\s*(millones?\s+de\s+pesos|millones?)",
                "currency_millions",
                1_000_000,
            ),
            (r"(?:\$|COP)\s*(\d+(?:[.,]\d+)?)\s*(millones?)?", "currency", 1_000_000),
            (
                r"(\d+(?:[.,]\d+)?)\s*(mil\s+millones?|billones?)",
                "currency_billions",
                1_000_000_000,
            ),
            (
                r"(\d+(?:[.,]\d+)?)\s*(personas?|beneficiarios?|familias?|hogares?)",
                "beneficiaries",
                1.0,
            ),
            (
                r"(\d+(?:[.,]\d+)?)\s*(hectáreas?|has?|km2?|metros?\s*cuadrados?)",
                "area",
                1.0,
            ),
            ...
            (
                r"d[ée]ficit\s+de\s+(\d+(?:[.,]\d+)?)\s*(%|por\s*ciento|personas?|millones?)?",
                "deficit",
                1.0,
            ),
            (r"brecha\s+de\s+(\d+(?:[.,]\d+)?)\s*(%|puntos?|millones?)?", "gap", 1.0),
            (
                r"falta(?:n)?\s+(\d+(?:[.,]\d+)?)\s*(personas?|millones?|%)?",
                "shortage",
                1.0,
            ),
            (
                r"sin\s+(?:acceso|cobertura|atenci[óo]n)\s*[:\s]+(\d+(?:[.,]\d+)?)\s*(%|personas?)?",
                "uncovered",
                1.0,
            ),
            (
    ...
    def _get_extended_context(
        self, text: str, start: int, end: int, window_size: int = 300
    ) -> str:
    ...
    def _analyze_regulatory_constraints(
        ...
        constraint_types_mentioned = sum(
            1 for types in constraint_types.values() if types
        )
        ...
        logger.info(
            f"D1-Q5 Regulatory Analysis: {constraint_types_mentioned} constraint types, "
        ...
        return {
            ...
            "constraint_types_detected": {
                k: len(v) for k, v in constraint_types.items()
            },
            ...
            "d1_q5_criteria": {
                ...
                "temporal_competency_constraints": len(constraint_types["Temporal"])
                > 0,
    ...
    def _calculate_comprehensive_severity(
        self, stmt_a: PolicyStatement, stmt_b: PolicyStatement, base_score: float
    ) -> float:
        ...
        entity_overlap = len(
            {e["text"] for e in stmt_a.entities} & {e["text"] for e in stmt_b.entities}
        )
        overlap_factor = min(1.3, 1.0 + entity_overlap * 0.1)

        has_quantitative = bool(
            stmt_a.quantitative_claims and stmt_b.quantitative_claims
        )
    ...
    def _generate_resolution_strategies(
        ...
        base_strategies = strategies.get(
            contradiction_type,
            [
    ...
    def _are_comparable_claims_advanced(
        ...
        tokens_a = {
            token.lemma_.lower() for token in doc_a if token.pos_ in ["NOUN", "VERB"]
        }
        tokens_b = {
            token.lemma_.lower() for token in doc_b if token.pos_ in ["NOUN", "VERB"]
        }
    ...
    def _extract_detailed_resource_mentions(
        ...
        patterns = [
            (
                r"SGP\s*(?:educación|salud|agua|propósito\s+general)?\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "SGP",
            ),
            (
                r"SGR\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "SGR",
            ),
            (
                r"regalías\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "regalías",
            ),
            (
                r"recursos?\s+propios\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "recursos_propios",
            ),
            (
                r"cofinanciación\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "cofinanciación",
            ),
            (
                r"crédito\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "crédito",
            ),
            (
                r"presupuesto\s+total\s*[:\s]*\$?\s*(\d+(?:[.,]\d+)?)\s*(millones?|mil\s+millones?)?",
                "presupuesto_total",
            ),
        ]
        ...
        for pattern, resource_type in patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                amount = (
                    self._parse_number_robust(match.group(1))
                    if match.group(1)
                    else None
                )
    ...
    def _deduplicate_contradictions(
        self, contradictions: List[ContradictionEvidence]
    ) -> List[ContradictionEvidence]:
    ...
    def _calculate_global_semantic_coherence_advanced(
        ...
        embeddings = [
            s.contextual_embedding or s.embedding
            for s in statements
        ...
        consecutive_sims = [
            similarity_matrix[i, i + 1] for i in range(len(similarity_matrix) - 1)
        ]
    ...
    def _calculate_objective_alignment_advanced(
        ...
        objective_statements = [
            s for s in statements if s.semantic_role in ["objective", "strategy"]
        ]
    ...
    def _calculate_syntactic_complexity_advanced(self, text: str) -> float:
        ...
        subordinate_clauses = len(
            [token for token in doc if token.dep_ in ["csubj", "ccomp", "advcl"]]
        )
        subordination_ratio = (
            subordinate_clauses / len(list(doc.sents)) if doc.sents else 0
        )
        ...
        for cont_type, conflicts in by_type.items():
            ...
            recommendation = {
                ...
                "affected_sections": list(
                    {f"Dim: {c.statement_a.dimension.value}" for c in conflicts}
                ),
                "estimated_effort": self._estimate_resolution_effort(
                    cont_type, len(conflicts)
                ),
            }
    ...
    def _get_recommendation_description(self, cont_type: ContradictionType) -> str:
        ...
        descriptions = {
            ContradictionType.NUMERICAL_INCONSISTENCY: "Reconciliar cifras inconsistentes mediante validación técnica con fuentes primarias",
            ContradictionType.TEMPORAL_CONFLICT: "Ajustar cronograma para resolver conflictos de precedencia y simultaneidad",
            ContradictionType.SEMANTIC_OPPOSITION: "Clarificar conceptos opuestos y establecer jerarquía clara de objetivos",
            ContradictionType.RESOURCE_ALLOCATION_MISMATCH: "Revisar asignación presupuestal para eliminar sobre-asignaciones o conflictos",
            ContradictionType.LOGICAL_INCOMPATIBILITY: "Resolver incompatibilidades lógicas mediante análisis de cadena de valor",
            ContradictionType.CAUSAL_INCOHERENCE: "Validar relaciones causales y eliminar circularidades o inconsistencias",
        }
        return descriptions.get(cont_type, "Revisar y ajustar según análisis técnico")

    def _estimate_resolution_effort(
        self, cont_type: ContradictionType, count: int
    ) -> str:
    ...
    def _serialize_contradiction(
        self, contradiction: ContradictionEvidence
    ) -> Dict[str, Any]:
        ...
        return {
            "statement_1": {
                ...
                "has_temporal_markers": bool(
                    contradiction.statement_a.temporal_markers
                ),
                "has_quantitative_claims": bool(
                    contradiction.statement_a.quantitative_claims
                ),
            ...
            "statement_2": {
                ...
                "has_temporal_markers": bool(
                    contradiction.statement_b.temporal_markers
                ),
                "has_quantitative_claims": bool(
                    contradiction.statement_b.quantitative_claims
                ),
    ...
    def _get_causal_network_statistics(self) -> Dict[str, Any]:
        ...
        if causal_net.number_of_nodes() == 0:
            return {"nodes": 0, "edges": 0, "cycles": 0, "avg_causal_strength": 0.0}
...
"""
    ...
    print(
        f"  - Coherencia semántica: {result['coherence_metrics']['semantic_coherence']:.3f}"
    )
    print(
        f"  - Consistencia temporal: {result['coherence_metrics']['temporal_consistency']:.3f}"
    )
    print(
        f"  - Coherencia causal: {result['coherence_metrics']['causal_coherence']:.3f}"
    )
    ...
    for i, rec in enumerate(result["recommendations"][:3], 1):
        ...
        print(
            f"   Cantidad: {rec['count']} | Severidad promedio: {rec['avg_severity']:.2f}"
        )
