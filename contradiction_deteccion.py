...
class BayesianCausalInference:
    ...
    def _check_temporal_precedence(
        ...
        cause_timestamps = [
            m.get("timestamp")
            for m in cause.temporal_markers
            if m.get("timestamp") is not None
        ]
        effect_timestamps = [
            m.get("timestamp")
            for m in effect.temporal_markers
            if m.get("timestamp") is not None
        ]
...
class GraphNeuralReasoningEngine:
    ...
    def _compute_contradiction_score(
        ...
        has_negation_a = any(
            neg in stmt_a.text.lower() for neg in ["no", "nunca", "ningún"]
        )
        has_negation_b = any(
            neg in stmt_b.text.lower() for neg in ["no", "nunca", "ningún"]
        )
...
class PolicyContradictionDetectorV2:
    ...
    def detect(
        ...
        audit_summary = {
            ...
            "quality_grade": (
                "Excelente"
                if self._audit_metrics.get("total_contradictions", len(contradictions))
                < 5
                else (
                    "Bueno"
                    if self._audit_metrics.get(
                        "total_contradictions", len(contradictions)
                    )
                    < 10
                    else "Regular"
        ...
        for i, stmt_a in enumerate(statements):
            for stmt_b in statements[i + 1 :]:
                ...
                if (
                    ...
                    confidence, credible_interval = (
                        self.bayesian_engine.calculate_posterior(
                ...
                if p_value < 0.05:
                    for i, (stmt_a, amount_a) in enumerate(allocations):
                        for stmt_b, amount_b in allocations[i + 1 :]:
                            rel_diff = abs(amount_a - amount_b) / max(
                                amount_a, amount_b
                            )
    ...
    def _calculate_advanced_coherence_metrics(
        ...
        confidence_interval = self._calculate_bootstrap_confidence_interval(
            coherence_score, len(statements), n_bootstrap=1000
        )
    ...
    def _calculate_comprehensive_severity(
        ...
        entity_overlap = len(
            set(e["text"] for e in stmt_a.entities)
    ...
    def _determine_relation_type_advanced(
        ...
        shared_entities = set(e["text"] for e in stmt_a.entities) & set(
        ...
        for cont_type, conflicts in by_type.items():
            ...
            recommendation = {
                ...
                "affected_sections": list(
                    set(f"Dim: {c.statement_a.dimension.value}" for c in conflicts)
                ),
