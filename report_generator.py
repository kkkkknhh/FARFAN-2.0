...
class ReportGenerator:
    ...
    def generate_micro_report(
        self, question_responses: Dict[str, Any], policy_code: str
    ) -> Dict:
        """
        ...
        logger.info(
            f"  Promedio general: {micro_report['statistics']['promedio_general']:.3f}"
        )

        return micro_report

    def generate_meso_report(
        self, question_responses: Dict[str, Any], policy_code: str
    ) -> Dict:
        """
            ...
            for dim_num in range(1, 7):
                ...
                for question_id, response in question_responses.items():
                    ...
                    if len(parts) >= 2:
                        ...
                        if (
                            self.punto_to_cluster.get(punto) == cluster
                            and dimension == dim_id
                        ):
        ...
        meso_report["statistics"] = {
            "promedio_clusters": (
                sum(all_dim_scores) / len(all_dim_scores) if all_dim_scores else 0
            ),
        ...
        logger.info(
            f"  Promedio clústeres: {meso_report['statistics']['promedio_clusters']:.3f}"
        )
    ...
    def generate_macro_report(
        ...
        if (
            SMART_AVAILABLE
            and recommendations
            and hasattr(recommendations[0], "to_dict")
        ):
            recommendations_data = [r.to_dict() for r in recommendations]
            recommendations_summary = [
                f"{r.id}: {r.title} (Prioridad: {r.priority.value}, AHP: {r.ahp_score}/10)"
        ...
        macro_report = {
            ...
            "debilidades_criticas": self._identify_critical_weaknesses(
                question_responses
            ),
    ...
    def _validate_narrative_coherence(
        self, global_score: float, responses: Dict
    ) -> Dict[str, Any]:
        """
        ...
        """
        coherence = {"is_coherent": True, "warnings": [], "validations": {}}
        ...
        dim_avg_global = (
            sum(dim_averages.values()) / len(dim_averages) if dim_averages else 0
        )
    ...
    def _get_cluster_puntos(self, cluster: ClusterMeso) -> List[str]:
        ...
        punto_lists = {
            ClusterMeso.C1_SEGURIDAD_PAZ: [
                "P1-Mujeres/Género",
                "P2-Prevención Violencia",
                "P8-Líderes DDHH",
            ],
            ClusterMeso.C2_DERECHOS_SOCIALES: [
                "P4-Derechos ESC",
                "P5-Víctimas/Paz",
                "P6-Niñez/Juventud",
            ],
    ...
    def _generate_dimension_observations(
        self, cluster: ClusterMeso, dim_id: str, responses: List, score: float
    ) -> str:
    ...
    def _generate_cluster_evaluation(
        self, cluster: ClusterMeso, dimensiones: Dict
    ) -> str:
...
"""
    ...
    def _identify_top_dimensions(self, dimensiones: Dict, n: int) -> str:
        ...
        sorted_dims = sorted(
            dimensiones.items(), key=lambda x: x[1]["score"], reverse=True
        )
    ...
    def _find_best_cluster(self, clusters: Dict) -> str:
        ...
        for cluster_id, cluster_data in clusters.items():
            if cluster_data["dimensiones"]:
                avg = sum(
                    d["score"] for d in cluster_data["dimensiones"].values()
                ) / len(cluster_data["dimensiones"])
                if avg > best_score:
                    best_score = avg
    ...
    def _extract_dimension_scores_from_clusters(
        self, clusters: Dict
    ) -> Dict[str, List[float]]:
    ...
    def _generate_retrospective_analysis(
        self, global_score: float, responses: Dict
    ) -> str:
...
"""
    ...
    def _generate_prospective_analysis(
        self, global_score: float, responses: Dict
    ) -> str:
...
"""
    ...
    def _extract_dimension_scores_from_responses(
        self, responses: Dict
    ) -> Dict[str, List[float]]:
    ...
    def _generate_priority_recommendations(
        self, responses: Dict, compliance_score: float
    ) -> List[Any]:
        """
        ...
        critical_questions = [
            (qid, r) for qid, r in responses.items() if r.nota_cuantitativa < 0.40
        ]
        ...
        # Add dependencies
        if len(recommendations) > 1 and any(
            r.priority == Priority.CRITICAL for r in recommendations
        ):
            # Other recommendations depend on critical ones
            critical_ids = [
                r.id for r in recommendations if r.priority == Priority.CRITICAL
            ]
    ...
    def _identify_strengths(self, responses: Dict) -> List[str]:
        ...
        excellent = [
            (qid, r) for qid, r in responses.items() if r.nota_cuantitativa >= 0.85
        ]
...
"""
        ...
        if SMART_AVAILABLE and "recomendaciones_summary" in macro_report:
            ...
            if (
                isinstance(macro_report["recomendaciones_prioritarias"], list)
                and macro_report["recomendaciones_prioritarias"]
                and isinstance(macro_report["recomendaciones_prioritarias"][0], dict)
            ):
