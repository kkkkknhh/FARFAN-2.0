...
def verify_imports():
    ...
    try:
        ...
        # Verify enums
        print_info(
            f"ValidationSeverity levels: {[s.value for s in ValidationSeverity]}"
        )
        print_info(
            f"ValidationDimension values: {[d.value for d in ValidationDimension]}"
        )
...
def verify_data_structures():
    ...
    try:
        ...
        print_success(
            f"PDMOntology created with {len(ontology.canonical_chain)} categories"
        )

        # Test SemanticChunk
        chunk = SemanticChunk(text="Test text", dimension="ESTRATEGICO")
        print_success(f"SemanticChunk created: {chunk.dimension}")

        # Test ExtractedTable
        table = ExtractedTable(
            title="Test Table", headers=["A", "B"], rows=[["1", "2"]]
        )
...
def verify_validator_initialization():
    ...
    try:
        from validators import AxiomaticValidator, PDMOntology, ValidationConfig
...
def verify_validation_result_methods():
    ...
    try:
        ...
        result.add_critical_failure(
            dimension="D6", question="Q2", evidence=[("A", "B")], impact="Test impact"
        )
        ...
        if result.failures:
            ...
            print_success(
                f"Failure details accessible: {failure.dimension}-{failure.question}"
            )
