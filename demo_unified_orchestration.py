    ...
    class MockPandas:
        class Timestamp:
            ...
            def now():
                ...
                return type(
                    "obj", (object,), {"isoformat": lambda: datetime.now().isoformat()}
                )()
...
class MockCausalBuilder:
    ...
    async def build_graph(self, chunks, tables):
        ...
        graph.add_edge(
            ...
            "salud_poblacional",
            weight=0.85,
            mechanism="administrativo",
        )
        graph.add_edge(
            "salud_poblacional", "calidad_vida", weight=0.75, mechanism="politico"
        )

        print(
            f"  [Graph] Created graph: {graph.number_of_nodes()} nodes, "
...
class MockBayesianEngine:
    ...
    async def infer_all_mechanisms(self, graph, chunks):
        ...
        print(
            f"  [Bayesian] Failed necessity tests: "
...
class MockScorer:
    ...
    def calculate_all_levels(
        ...
        meso_scores = {"C1": 0.72, "C2": 0.68, "C3": 0.75, "C4": 0.70}
        ...
        return {"micro": micro_scores, "meso": meso_scores, "macro": macro_score}
...
async def main():
    ...
    failed = sum(
        1 for m in result.mechanism_results if not m.necessity_test.get("passed", True)
    )
    ...
    if result.validation_result:
        ...
        print(
            f"  - Contradiction density: {result.validation_result.contradiction_density:.3f}"
        )
    ...
    for metric in result.stage_metrics:
        print(
            f"  - {metric.stage.name}: {metric.duration_seconds:.3f}s "
