...
class BayesianPriorBuilder:
    ...
    def _compute_beta_params(
        ...
        if self.structural_penalty_factor < 1.0:
            ...
            self.logger.debug(
                ...
                alpha,
                beta,
                self.structural_penalty_factor,
            )

        return alpha, beta

    def apply_structural_penalty(
        self, penalty_factor: float, violations: List[Tuple[str, str]]
    ):
        """
    ...
    def _apply_penalty_to_prior(self, alpha: float, beta: float) -> Tuple[float, float]:
        """
