...
class TestD6AuditModule(unittest.TestCase):
    ...
    def test_d6_q1_axiomatic_validation_complete_graph(self):
        ...
        # Check criteria
        self.assertTrue(result.has_five_elements, 
                       "Should have all 5 elements")
        self.assertEqual(len(result.elements_present), 5,
                        "Should have 5 elements present")
        self.assertEqual(len(result.elements_missing), 0,
                        "Should have no missing elements")
        self.assertTrue(result.violaciones_orden_empty,
                       "Should have no order violations")
        self.assertTrue(result.caminos_completos_exist,
                       "Should have at least one complete path")
    ...
    def test_d6_q1_axiomatic_validation_incomplete_graph(self):
        ...
        self.assertFalse(result.has_five_elements,
                        "Should not have all 5 elements")
        self.assertTrue(len(result.elements_missing) > 0,
                       "Should have missing elements")
        
        # Quality should not be Excelente
        self.assertNotEqual(result.quality_grade, 'Excelente',
    ...
    def test_d6_q3_high_incoherence(self):
        ...
        self.assertFalse(result.flags_below_limit,
                        "Should not be below limit with 7 flags")
        
        # Quality should be Regular
        self.assertEqual(result.quality_grade, 'Regular',
                        "Should get Regular with >= 5 flags")
    ...
    def test_d6_q4_adaptive_me_with_learning(self):
        ...
        self.assertTrue(result.has_correction_mechanism,
                       "Should detect correction mechanism")
        self.assertTrue(result.has_feedback_mechanism,
                       "Should detect feedback mechanism")
        self.assertTrue(result.prior_updates_detected,
                       "Should detect prior updates")
        self.assertIsNotNone(result.uncertainty_reduction,
                            "Should calculate uncertainty reduction")
        
        # Uncertainty reduction should be positive
        if result.uncertainty_reduction is not None:
            self.assertGreaterEqual(result.uncertainty_reduction, 0,
    ...
    def test_d1_q5_d6_q5_restrictions_analysis(self):
        ...
        self.assertGreaterEqual(result.restriction_count, 3,
                               "Should detect at least 3 restriction types")
        ...
        self.assertTrue(len(result.legal_constraints) > 0,
                       "Should detect legal constraints")
        
        # Quality should be Excelente (≥3 restrictions + temporal consistency)
        self.assertIn(result.quality_grade, ['Excelente', 'Bueno'],
    ...
    def test_full_audit_execution(self):
        ...
        self.assertIsInstance(report.d1_q5_d6_q5_restrictions, D1Q5D6Q5RestrictionsResult)
    ...
    def test_overall_assessment_calculation(self):
        ...
        d6_q1 = D6Q1AxiomaticResult(
            ...
            elements_present=['INSUMOS', 'PROCESOS', 'PRODUCTOS', 'RESULTADOS', 'CAUSALIDAD'],
            elements_missing=[],
