...
def ejemplo_basico():
    ...
    checkpoint = PipelineCheckpoint(
        checkpoint_dir="./checkpoints_ejemplo",
...
def ejemplo_encriptacion():
    ...
    try:
        ...
        print(f"  Presupuesto total: ${restored['financial_data']['presupuesto_total']:,}")
        print(f"  Población víctimas: {restored['sensitive_indicators']['poblacion_victimas']:,}")
...
def ejemplo_consultas():
    ...
    checkpoint = PipelineCheckpoint(
        checkpoint_dir="./checkpoints_queries",
        enable_incremental=True
    )
...
def ejemplo_workflow_completo():
    ...
    print("\n[STAGE 4] DNP Validation")
    state.update({
        'dnp_compliance_score': 0.87,
        'competencias_validadas': ['EDU', 'SAL', 'INF']
    })
