...
def enforce_no_magic_numbers():
    ...
    try:
        ...
        allowed_literals = {0, 1, 0.0, 1.0, 100, 100.0, 200}  # 200 is for text truncation
        violations = []
    ...
    for text in test_cases:
        ...
        if reg.get("status") == "success":
            ...
            if not isinstance(score_adjusted, (int, float)):
                raise ContractViolation(f"score_adjusted is not numeric: {type(score_adjusted)}")
            
            # Verify bounds
            if not (0.0 <= score_raw <= 1.0):
                raise ContractViolation(f"score_raw out of bounds: {score_raw}")
            if not (0.0 <= score_adjusted <= 1.0):
                raise ContractViolation(f"score_adjusted out of bounds: {score_adjusted}")
