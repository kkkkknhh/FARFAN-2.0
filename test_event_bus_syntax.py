...
def main():
    ...
    for name, complexity in sorted(complexities.items()):
        if name in ['publish', '_prepare_handler_task', '_collect_handler_tasks', '_log_handler_exceptions']:
    ...
    helper_methods = ['_prepare_handler_task', '_collect_handler_tasks', '_log_handler_exceptions']
    ...
    if publish_complexity <= 15:
        ...
        print(f"✗ Complexity still too high (target: ≤15, actual: {publish_complexity})")
        return 1
