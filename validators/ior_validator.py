...
class IoRValidator:
    ...
    def verify_provenance_traceability(
        ...
        for chunk in chunks:
            ...
            if stored_hash == recomputed_hash:
                ...
                hash_mismatches += 1
                self.logger.warning(
                    f"Hash mismatch for chunk {chunk.chunk_id}: "
                    f"stored != recomputed"
                )
        ...
        # Log summary
        verification_rate = (
            (verified_hashes / len(chunks) * 100) if chunks else 0
        )
    ...
    def generate_ior_audit_report(self) -> Dict[str, Any]:
        ...
        passed_validations = sum(
            1 for v in self._validation_history if v.passed
        )
        
        # Aggregate provenance checks
        total_provenance = len(self._provenance_history)
        passed_provenance = sum(
            1 for p in self._provenance_history if p.passed
        )
        
        # Aggregate financial checks
        total_financial = len(self._financial_history)
        passed_financial = sum(
            1 for f in self._financial_history if f.passed
        )
        ...
        report = {
            ...
            "audit_point_1_1_schema_enforcement": {
                ...
                "pass_rate": round(
                    ...
                    if total_validations > 0 else 0, 2
                ),
            ...
            "audit_point_1_2_provenance_traceability": {
                ...
                "pass_rate": round(
                    ...
                    if total_provenance > 0 else 0, 2
                ),
