...
class AxiomaticValidator:
    ...
    def validate_complete(
        ...
        logger.info("  → Mapping to canonical scores: D6-Q1 (completeness), D6-Q2 (causal order)")
        ...
        if d6_q1_score < 0.55 or d6_q2_score < 0.55:
            ...
            logger.warning("⚠ GOVERNANCE TRIGGER 2: D6 scores below 0.55 - blocking progression")
            results.add_critical_failure(
                ...
                recommendations=['Strengthen causal model completeness and ordering before proceeding']
            )
        ...
        if structural.violaciones_orden:
            ...
            if self.config.enable_structural_penalty:
                ...
                logger.info("  Applied structural penalty factor: %.3f to Bayesian posteriors", 
                           penalty_factor)
        ...
        logger.info("  → Mapping to canonical scores: D2-Q5 (design contradictions), D6-Q3 (causal coherence)")
        ...
        d6_q3_score = self._calculate_d6_q3_score(results.contradiction_density, contradictions)
        ...
        if results.contradiction_density > self.config.contradiction_threshold:
            if self.config.enable_human_gating:
                ...
                logger.warning("⚠ GOVERNANCE TRIGGER 1: Contradiction density %.4f > %.4f - flagging for manual review",
                             results.contradiction_density, self.config.contradiction_threshold)
                results.add_critical_failure(
                    dimension='D2',
                    question='Q5',
                    evidence={'density': results.contradiction_density, 'count': len(contradictions)},
        ...
        if d6_q3_score < 0.55:
            results.requires_manual_review = True
            results.hold_reason = 'D6_SCORE_BELOW_THRESHOLD'
            logger.warning("⚠ GOVERNANCE TRIGGER 2: D6-Q3 score %.3f < 0.55 - blocking progression", 
                         d6_q3_score)
        ...
        logger.info("  → Mapping to canonical scores: D1-Q5 (DNP diagnostic compliance), D4-Q5 (results validation)")
        ...
        results.is_valid = (
            ...
            all(score >= 0.55 for key, score in results.score_mappings.items() if 'D6' in key)
        )
    ...
    def _validate_semantic(
        ...
        try:
            ...
            logger.debug("Semantic validation found %d contradictions", len(contradictions))
    ...
    def _apply_structural_penalty(
        ...
        for source, target in violations:
            if causal_graph.has_edge(source, target):
                ...
                causal_graph.edges[source, target]['violation_reason'] = 'structural_order'
                ...
                if 'confidence' in causal_graph.edges[source, target]:
                    ...
                    causal_graph.edges[source, target]['confidence'] = original * penalty_factor
    ...
    def _calculate_d6_q1_score(self, structural_result: 'ValidacionResultado') -> float:
        ...
        category_score = max(0.0, (expected_categories - missing_count) / expected_categories)
    ...
    def _calculate_d2_q5_score(
        ...
        design_contradictions = [
            ...
            if 'dimension' in c and c['dimension'] in ['ESTRATEGICO', 'PROGRAMATICO', 'DIAGNOSTICO']
        ]
    ...
    def _calculate_d6_q3_score(
        ...
        severity_factor = len(causal_contradictions) / max(1, len(contradictions)) if contradictions else 0.0
    ...
    def _calculate_d1_q5_score(self, dnp_result: Optional['ResultadoValidacionDNP']) -> float:
        """
