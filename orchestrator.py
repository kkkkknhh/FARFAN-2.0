...
class PhaseInput:
    ...
    def __post_init__(self):
        ...
        assert isinstance(self.text, str) and len(self.text) > 0, "text must be non-empty string"
        assert isinstance(self.plan_name, str) and len(self.plan_name) > 0, "plan_name must be non-empty string"
        assert isinstance(self.dimension, str) and len(self.dimension) > 0, "dimension must be non-empty string"
        assert isinstance(self.trace_id, str) and len(self.trace_id) > 0, "trace_id must be non-empty string"
...
class AnalyticalOrchestrator:
    ...
    def __init__(
        ...
        if CONTRADICTION_DETECTOR_AVAILABLE:
            try:
                ...
                self.logger.error(f"FATAL: Failed to initialize PolicyContradictionDetectorV2: {e}")
                raise RuntimeError(f"Module initialization failure: PolicyContradictionDetectorV2: {e}")
        else:
            self.logger.warning("⚠ PolicyContradictionDetectorV2 not available - module will operate in degraded mode")
        ...
        if DNP_VALIDATOR_AVAILABLE:
            try:
                ...
                self.logger.warning(f"ValidadorDNP initialization failed (missing dependencies): {e}")
                self.logger.warning("⚠ ValidadorDNP not available - module will operate in degraded mode")
        ...
        if TEORIA_CAMBIO_AVAILABLE:
            ...
            self.logger.warning("⚠ TeoriaCambio not available - module will operate in degraded mode")
        ...
        if BAYESIAN_ENGINE_AVAILABLE:
            try:
                ...
                self.logger.error(f"FATAL: Failed to initialize BayesianPriorBuilder: {e}")
                raise RuntimeError(f"Module initialization failure: BayesianPriorBuilder: {e}")
        else:
            self.logger.warning("⚠ BayesianPriorBuilder not available - module will operate in degraded mode")
    ...
    def orchestrate_analysis(
        ...
        self.logger.info(f"Starting orchestration: trace_id={trace_id}, sha256={sha256_source[:16]}...")
        ...
        try:
            ...
            self._emit_phase_telemetry("extract_statements", statements_result, trace_id)
            ...
            self._emit_phase_telemetry("detect_contradictions", contradictions_result, trace_id)
            ...
            self._emit_phase_telemetry("analyze_regulatory_constraints", regulatory_result, trace_id)
            ...
            self._emit_phase_telemetry("validate_regulatory", validation_result, trace_id)
            ...
            self._emit_phase_telemetry("calculate_coherence_metrics", coherence_result, trace_id)
            ...
            self._emit_phase_telemetry("generate_recommendations", recommendations_result, trace_id)
            ...
            self.logger.info(f"[{trace_id}] Pipeline completed successfully in {duration:.2f}s")
            ...
            raise RuntimeError(f"Orchestration failed for trace_id={trace_id}: {e}") from e
    ...
    def _extract_statements(
        ...
        if not self.contradiction_detector:
            raise RuntimeError("PolicyContradictionDetectorV2 not available - cannot extract statements")
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Invalid dimension '{phase_input.dimension}', defaulting to ESTRATEGICO")
            ...
            assert all(hasattr(s, 'text') for s in statements), "all statements must have 'text' attribute"
            ...
            return PhaseResult(
                ...
                metrics={
                    ...
                    "avg_statement_length": sum(len(s.text) for s in statements) / len(statements) if statements else 0
                },
                timestamp=timestamp
            )
    ...
    def _detect_contradictions(
        ...
        try:
            ...
            detection_result = self.contradiction_detector.detect(
                phase_input.text,
                phase_input.plan_name,
                policy_dim
            )
            
            # Validate output contract
            assert isinstance(detection_result, dict), "detection_result must be a dictionary"
            assert "contradictions" in detection_result, "contradictions key required in output"
            assert "total_contradictions" in detection_result, "total_contradictions key required in output"
            ...
            for contradiction in contradictions:
                if isinstance(contradiction, dict) and contradiction.get("contradiction_type") == "TEMPORAL_CONFLICT":
                    temporal_conflicts.append(contradiction)
            
            # Calculate severity metrics
            critical_count = sum(1 for c in contradictions if isinstance(c, dict) and c.get("severity", 0) > self.calibration.CRITICAL_SEVERITY_THRESHOLD)
            high_count = sum(1 for c in contradictions if isinstance(c, dict) and self.calibration.HIGH_SEVERITY_THRESHOLD < c.get("severity", 0) <= self.calibration.CRITICAL_SEVERITY_THRESHOLD)
            medium_count = sum(1 for c in contradictions if isinstance(c, dict) and self.calibration.MEDIUM_SEVERITY_THRESHOLD < c.get("severity", 0) <= self.calibration.HIGH_SEVERITY_THRESHOLD)
    ...
    def _analyze_regulatory_constraints(
        ...
        try:
            ...
            validation_result = self.dnp_validator.validar_proyecto_integral(
                sector=sector,
                descripcion=phase_input.text[:500],  # Use first 500 chars as description
            ...
            # Validate output contract
            assert isinstance(validation_result, ResultadoValidacionDNP), "validation_result must be ResultadoValidacionDNP"
            
            # Build regulatory analysis output
            regulatory_analysis = {
                "regulatory_references_count": len(set(regulatory_references)),
                "constraint_types_mentioned": len(validation_result.sectores_intervenidos),
            ...
            self.logger.error(f"Regulatory constraints analysis failed: {e}", exc_info=True)
            # SIN_CARRETA: NO SILENT FAILURES
            raise RuntimeError(f"Phase analyze_regulatory_constraints failed: {e}") from e
    ...
    def _validate_regulatory(
        ...
        try:
            ...
            for i, stmt in enumerate(statements[:10]):  # Limit to first 10 for performance
                node_id = f"stmt_{i}"
            ...
            nodos_con_categoria = {}
            for i, node in enumerate(list(causal_graph.nodes())[:5]):  # Limit validation
            ...
            if nodos_con_categoria:
                ...
                validation_result = ValidacionResultado(
                    ...
                    sugerencias=["No hay suficientes declaraciones para validar estructura causal"]
                )
            
            # Validate output contract
            assert isinstance(validation_result, ValidacionResultado), "validation_result must be ValidacionResultado"
            ...
            return PhaseResult(
                ...
                outputs={
                    "teoria_cambio_validation": {
                        ...
                        "categorias_faltantes": [c.name for c in validation_result.categorias_faltantes],
    ...
    def _calculate_coherence_metrics(
        ...
        try:
            ...
            temporal_contradictions = [c for c in contradictions if isinstance(c, dict) and c.get("contradiction_type") == "TEMPORAL_CONFLICT"]
            temporal_consistency = max(0.0, 1.0 - (len(temporal_contradictions) / max(1, len(statements))))
            
            # Calculate causal coherence
            causal_contradictions = [c for c in contradictions if isinstance(c, dict) and c.get("contradiction_type") == "CAUSAL_INCOHERENCE"]
            causal_coherence = max(0.0, 1.0 - (len(causal_contradictions) / max(1, len(statements))))
            
            # Determine quality grade based on coherence threshold
            meets_threshold = overall_coherence_score >= self.calibration.COHERENCE_THRESHOLD
            ...
            # Validate output contract
            assert isinstance(coherence_metrics, dict), "coherence_metrics must be a dictionary"
            assert "overall_coherence_score" in coherence_metrics, "overall_coherence_score required"
            assert 0.0 <= coherence_metrics["overall_coherence_score"] <= 1.0, "coherence score must be in [0, 1]"
            ...
            self.logger.error(f"Coherence metrics calculation failed: {e}", exc_info=True)
            # SIN_CARRETA: NO SILENT FAILURES
    ...
    def _generate_audit_summary(
        ...
        try:
            ...
            causal_incoherence_count = sum(
                ...
                if isinstance(c, dict) and c.get("contradiction_type") == "CAUSAL_INCOHERENCE"
            )
            
            # Count structural failures
            structural_failures = sum(
                1 for c in contradictions
                if isinstance(c, dict) and c.get("contradiction_type") in ["LOGICAL_INCOMPATIBILITY", "OBJECTIVE_MISALIGNMENT"]
            )
            ...
            assert "total_contradictions" in audit_summary, "total_contradictions required"
            assert audit_summary["total_contradictions"] >= 0, "total_contradictions must be non-negative"
            ...
            return PhaseResult(
                ...
                metrics={
                    ...
                    "structural_failures": structural_failures,
                    "meets_causal_limit": 1.0 if audit_summary["meets_causal_limit"] else 0.0
                },
                timestamp=timestamp
            )
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            return PhaseResult(
                ...
                inputs={
                    "contradictions_count": len(contradictions),
                    "regulatory_score": regulatory_outputs.get("d1_q5_regulatory_analysis", {}).get("dnp_score", 0),
                    "trace_id": phase_input.trace_id
                },
                outputs={
                ...
                metrics={
                    ...
                    "avg_ahp_score": sum(rec.ahp_score for rec in recommendations) / len(recommendations) if recommendations else 0,
                    "critical_count": sum(1 for rec in recommendations if rec.priority == Priority.CRITICAL),
                    "high_count": sum(1 for rec in recommendations if rec.priority == Priority.HIGH)
                },
                timestamp=timestamp
            )
