...
def test_orchestrator_initialization():
    ...
    assert orchestrator.circuit_breaker_registry is not None, "CircuitBreakerRegistry not initialized"
...
def test_stage_execution_flow():
    ...
    try:
        ...
        print(f"✓ Metrics recorded for {len(orchestrator.metrics.current_trace.stages)} stage(s)")
...
def test_failure_handling():
    ...
    assert len(orchestrator.risk_registry.mitigation_history) > 0, "No mitigation attempted"
    print(f"✓ Mitigation history: {len(orchestrator.risk_registry.mitigation_history)} attempt(s)")
    ...
    critical_alerts = [a for a in orchestrator.metrics.current_trace.alerts if a.level.value == "CRITICAL"]
...
def test_circuit_breaker_integration():
    ...
    print(f"✓ Circuit breaker stats: {stats['total_failures']} failures, state={stats['state']}")
