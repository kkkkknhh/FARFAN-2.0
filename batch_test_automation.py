...
class AutomatedWorkflow:
    ...
    def __init__(self, batch_size: int = 5):
        ...
        self.critical_failure_threshold = (
            0.1  # 10% test failure triggers rollback warning
        )
    ...
    def record_and_test(self, file_path: str, description: str = "") -> dict:
        ...
        edits_in_batch = (
            len(self.tracker.current_batch.edits) if self.tracker.current_batch else 0
        )
    ...
    def _analyze_test_results(self, test_results: dict, batch_id: int) -> dict:
                ...
                failed_modules.append(
                    {
                        ...
                        "failure_rate": (
                            module_failed / result.total_tests
        ...
        return {
            ...
            "failed_modules": sorted(
                failed_modules, key=lambda x: x["failed"], reverse=True
            ),
    ...
    def _handle_critical_failure(self, batch_id: int, analysis: dict):
        ...
        print(
            f"Failure rate: {analysis['failure_rate'] * 100:.1f}% ({analysis['total_failed']}/{analysis['total_tests']} tests)"
        )
        print(f"\nMost affected modules:")

        for mod in analysis["failed_modules"][:5]:
            print(
                f"  • {mod['module']}: {mod['failed']}/{mod['total']} tests failed ({mod['failure_rate'] * 100:.1f}%)"
            )
        ...
        print(
            f"2. Review detailed failures in test_batch_logs/test_report_batch_{batch_id}.txt"
        )
        print(
            f"3. Consider rollback: python batch_test_automation.py rollback {batch_id}"
        )
        print("=" * 80)
    ...
    def _save_critical_failure_report(self, batch_id: int, analysis: dict):
        ...
        report = {
            ...
            "rollback_available": (
                self.tracker.BACKUP_DIR / f"batch_{batch_id}"
            ).exists(),
            "batch_edits": [],
        }
    ...
    def get_status(self):
        ...
        if self.tracker.test_results:
            ...
            for result in self.tracker.test_results[-5:]:
                ...
                print(
                    f"Batch {result.batch_id}: {status} - {result.passed_tests}/{result.total_tests} tests ({result.duration_seconds:.1f}s)"
                )
    ...
    if args.command == "record":
        ...
        if args.batch_id <= len(workflow.tracker.all_batches):
            ...
            test_result = next(
                (
                    r
                    for r in workflow.tracker.test_results
