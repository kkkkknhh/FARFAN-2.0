...
def test_audit_point_5_3_append_audit_record():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        record_metadata = orchestrator.append_audit_record(
            run_id=run_id, analysis_results=analysis_results, source_text=source_text
        )
        ...
        assert abs(actual_days - expected_days) <= 1, (
            f"Retention period mismatch: {actual_days} days"
        )
...
def test_audit_point_5_1_prior_learning_functionality():
    ...
    counterfactual_audit = {
        "causal_implications": {
            "MP-001": {
                "causal_effects": {"implementation_failure": True}  # Flagged as failure
            }
        }
    }

    audit_results = {"MP-001": {"passed": False}, "MP-002": {"passed": True}}

    # Mock _extract_feedback_from_audit logic
    print("\n2. Extracting feedback data...")
    feedback = extract_feedback_mock(
    ...
    assert "administrativo" in feedback["penalty_factors"], (
        "Failed mechanism type not penalized"
    )
    ...
    assert feedback["test_failures"]["necessity_failures"] > 0, (
        "Necessity failures not tracked"
    )
    assert feedback["test_failures"]["sufficiency_failures"] > 0, (
        "Sufficiency failures not tracked"
    )
    print(f"   ✓ Necessity failures: {feedback['test_failures']['necessity_failures']}")
    print(
        f"   ✓ Sufficiency failures: {feedback['test_failures']['sufficiency_failures']}"
    )
...
def test_audit_point_5_2_measurable_prior_decay():
    ...
    # 15% reduction per failure (from dereck_beach logic)
    penalty_factor = 0.85
    ...
    assert decay_rate > 20.0, (
        f"Decay rate {decay_rate:.2f}% does not meet >20% criterion"
    )
    ...
    print(
        f"   ✓ Priors decreased monotonically over {len(prior_history) - 1} iterations"
    )
...
def extract_feedback_mock(
    ...
    if total_mechanisms > 0:
        mechanism_frequencies = {
            k: v / total_mechanisms for k, v in mechanism_frequencies.items()
        }
    ...
    if total_failures > 0:
        failure_frequencies = {
            k: v / total_failures for k, v in failure_frequencies.items()
        }
...
def run_all_tests():
    ...
    tests = [
        ...
        (
            "5.1",
            "Prior Learning Functionality",
            test_audit_point_5_1_prior_learning_functionality,
        ),
