...
class RiskRegistry:
    ...
    def _initialize_default_risks(self):
        ...
        self.register_risk(RiskDefinition(
            ...
            applicable_stages=["SEMANTIC_ANALYSIS", "CAUSAL_EXTRACTION", "MECHANISM_INFERENCE"],
    ...
    def _initialize_mitigation_strategies(self):
        ...
        self.mitigation_strategies["reduce_confidence_threshold"] = self._strategy_reduce_threshold
        ...
        self.mitigation_strategies["estimate_from_text"] = self._strategy_estimate_financial
        self.mitigation_strategies["lower_extraction_threshold"] = self._strategy_lower_threshold
    ...
    def assess_stage_risks(self, stage_name: str, context: Any = None) -> List[RiskAssessment]:
        """
            ...
            if stage_name in risk.applicable_stages:
                ...
                if risk.detection_condition and context:
                    try:
                        ...
                        logger.warning(f"Error evaluando condición de riesgo {risk.risk_id}: {e}")
        ...
        critical_risks = [a for a in assessments if a.applicable and 
                         a.severity in [RiskSeverity.CRITICAL, RiskSeverity.HIGH]]
        if critical_risks:
            logger.warning(f"Stage {stage_name}: {len(critical_risks)} riesgos críticos/altos detectados")
        
        return assessments
    
    def find_risk_by_exception(self, exception: Exception, stage_name: str) -> Optional[RiskDefinition]:
        """
    ...
    def execute_mitigation(self, risk: RiskDefinition, context: Any = None) -> MitigationAttempt:
        """
        ...
        """
        logger.info(f"Ejecutando mitigación para riesgo: {risk.risk_id} (estrategia: {risk.mitigation_strategy})")
