...
class TestInputSchemaEnforcement:
    ...
    def test_evidence_pool_exclusion_on_validation_failure(self):
        ...
        invalid_tables = [
            {"data": [], "page_number": 1, "confidence_score": 0.5, "column_count": 0, "row_count": 0},
            {"data": [["A"]], "page_number": -1, "confidence_score": 0.5, "column_count": 1, "row_count": 1},
        ]
        ...
        assert len(validated_tables) == 0, "No invalid tables should enter evidence pool"
        assert len(rejection_log) == 2, "All invalid tables should be logged"
        
        logger.info(f"✓ Evidence pool exclusion verified: {len(rejection_log)} rejections logged")
...
class TestFinancialAnchorIntegrity:
    ...
    def test_financial_allocation_tracing(self):
        ...
        from dereck_beach import ConfigLoader, MetaNode, FinancialAuditor
        ...
        try:
            ...
            nodes = {
                ...
                'MP-002': MetaNode(
                    id='MP-002',
                    text='Mejoramiento educación rural',
                    type='producto'
                ),
            }
            ...
            assert 'MP-001' in auditor.financial_data or 'MP-002' in auditor.financial_data
            
            logger.info(f"✓ Financial allocations traced: {len(auditor.financial_data)} nodes")
    ...
    def test_ppi_bpin_code_extraction(self):
        ...
        try:
            ...
            nodes = {
                'MP-001': MetaNode(
                    id='MP-001',
                    text='Construcción acueducto rural',
                    type='producto'
                ),
            }
    ...
    def test_high_confidence_financial_matching(self):
        ...
        try:
            ...
            nodes = {
                'MP-001': MetaNode(
                    id='MP-001',
                    text='Meta producto 1',
                    type='producto'
                ),
                'MP-002': MetaNode(
                    id='MP-002',
                    text='Meta producto 2',
                    type='producto'
                ),
            }
            ...
            assert confidence >= 0.8, f"Financial match confidence {confidence:.2%} should be >= 80%"
...
class TestIoRIntegration:
    ...
    def test_rejection_and_logging_flow(self):
        ...
        invalid_inputs = [
            {"type": "table", "data": [], "page_number": 1, "confidence_score": 0.5, "column_count": 0, "row_count": 0},
            {"type": "chunk", "chunk_id": "", "text": "test", "start_char": 0, "end_char": 4, "doc_id": "abc"},
        ]
        ...
        for inp in invalid_inputs:
            try:
                if inp["type"] == "table":
                    ExtractedTable.model_validate({k: v for k, v in inp.items() if k != "type"})
                elif inp["type"] == "chunk":
                    SemanticChunk.model_validate({k: v for k, v in inp.items() if k != "type"})
            except ValidationError as e:
