...
class PipelineMetrics:
    ...
    def end_stage(self, success: bool, error_message: Optional[str] = None):
        ...
        logger.info(
            f"{status} Stage completed: {stage_name} ({self.current_stage.execution_time_ms:.1f}ms)"
        )
    ...
    def end_execution(self, success: bool):
        ...
        logger.info(
            f"📊 Ejecución finalizada: {self.current_trace.policy_code} ({self.current_trace.total_execution_time_ms:.1f}ms)"
        )
    ...
    def print_summary(self):
        ...
        print(
            f"✓  Etapas completadas: {len([s for s in self.current_trace.stages if s.success])}/{len(self.current_trace.stages)}"
        )
