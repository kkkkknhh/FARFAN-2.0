...
def config():
    ...
    @dc
    class MockConfig:
        self_reflection: MockSelfReflection = dc_field(
            default_factory=MockSelfReflection
        )
...
async def test_scorer_contract_violation_missing_keys(orchestrator, test_pdf_path):
    ...
    extraction_pipeline = AsyncMock()
    extraction_pipeline.extract_complete = AsyncMock(
        return_value={
    ...
    bad_scorer.calculate_all_levels = MagicMock(
        return_value={
            "micro": {},
            "meso": {},
...
async def test_deterministic_execution_same_inputs(config, test_pdf_path):
    ...
    def create_mock_components():
        ...
        bayesian_engine = AsyncMock()
        bayesian_engine.infer_all_mechanisms = AsyncMock(
            return_value=[
        ...
        scorer = MagicMock()
        scorer.calculate_all_levels = MagicMock(
            return_value={
...
async def test_telemetry_events_published(orchestrator, test_pdf_path, caplog):
    ...
    extraction_pipeline = AsyncMock()
    extraction_pipeline.extract_complete = AsyncMock(
        return_value={"semantic_chunks": [{"text": "test"}], "tables": []}
    )
    ...
    scorer = MagicMock()
    scorer.calculate_all_levels = MagicMock(
        return_value={"micro": {}, "meso": {}, "macro": 0.7}
    )
    ...
    assert (
        "[TELEMETRY] Pipeline SUCCESS" in log_text
        or "[TELEMETRY] Pipeline COMPLETE" in log_text
    )
