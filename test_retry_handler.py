...
def test_basic_retry():
    ...
    handler.configure(
        ...
        RetryConfig(base_delay=0.1, max_retries=3, failure_threshold=5),
    )
...
def test_retry_exhaustion():
    ...
    handler.configure(
        ...
        RetryConfig(base_delay=0.05, max_retries=2, failure_threshold=3),
    )
...
def test_circuit_breaker_open():
    ...
    handler.configure(
        ...
        RetryConfig(
            base_delay=0.05, max_retries=1, failure_threshold=3, recovery_timeout=1.0
        ),
    )
...
def test_context_manager():
    ...
    handler.configure(
        DependencyType.SPACY_MODEL, RetryConfig(base_delay=0.05, max_retries=2)
    )
...
def test_retry_history():
    ...
    for record in history[-3:]:
        print(
            f"  {record['dependency']}.{record['operation']}: "
