...
class QuestionnaireParser:
    ...
    def _parse(self) -> None:
        ...
        for i, match in enumerate(policy_matches):
            ...
            for j, dim_match in enumerate(dimension_matches):
                ...
                dim_end = (
                    dimension_matches[j + 1].start()
                    if j + 1 < len(dimension_matches)
                    else len(policy_content)
                )
    ...
    def get_questions_by_dimension(
        self, policy_id: str, dimension_id: str
    ) -> List[Question]:
        """
...
def create_questionnaire_parser(
