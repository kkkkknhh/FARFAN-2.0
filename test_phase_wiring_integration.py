...
class TestOrchestratorInitialization:
    ...
    def test_orchestrator_creates_with_defaults(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                orch.calibration.COHERENCE_THRESHOLD == CALIBRATION.COHERENCE_THRESHOLD
            )
            assert (
                orch.calibration.CAUSAL_INCOHERENCE_LIMIT
    ...
    def test_orchestrator_logs_module_availability(self, caplog):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                "Module initialization complete" in log_messages
                or "degraded mode" in log_messages
            )
    ...
    def test_extract_statements_requires_detector(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if orch.contradiction_detector is None:
                ...
                with pytest.raises(
                    RuntimeError, match="PolicyContradictionDetectorV2 not available"
                ):
...
class TestCalibrationConstantUsage:
    ...
    def test_audit_summary_uses_calibration_limits(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                result.inputs["causal_incoherence_limit"]
                == CALIBRATION.CAUSAL_INCOHERENCE_LIMIT
            )
