...
class TestPerformanceProfiling(unittest.TestCase):
    ...
    def test_semantic_chunking_performance(self):
        ...
        self.assertGreater(chars_per_sec, 1000, 
        ...
        if avg_chunk_time > 0.01:  # >10ms per chunk
            ...
            print("   Recommendation: Pre-compile regex patterns, optimize sentence detection")
    ...
    def test_semantic_chunking_algorithm_complexity(self):
        ...
        for size in sizes:
            ...
            start_time = time.time()
            chunks = asyncio.run(pipeline._chunk_with_provenance(test_text[:size], doc_id))
        ...
        if len(timings) >= 2:
            ...
            self.assertLess(linearity, 2.5, 
                           "Chunking should be approximately O(n)")
    ...
    def test_performance_regression_detection(self):
        ...
        print(f"Chunking throughput (c/s)     {BASELINE_METRICS['chunking_throughput_chars_per_sec']:>8} {throughput:>10.0f}    ", end="")
        if throughput >= BASELINE_METRICS['chunking_throughput_chars_per_sec']:
            print("✓ PASS")
        else:
            print("✗ FAIL (regression)")
        
        print(f"Chunk ID generation (ms)      {BASELINE_METRICS['chunk_id_generation_ms']:>8.2f} {chunk_id_time:>10.3f}    ", end="")
