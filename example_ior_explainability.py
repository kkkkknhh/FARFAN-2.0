...
def example_1_basic_explainability_payload():
    ...
    print(
        f"  95% Credible Interval: ({posterior.credible_interval_95[0]:.3f}, {posterior.credible_interval_95[1]:.3f})"
    )
    print()
    ...
    print(
        f"  Credible Interval: ({payload.credible_interval_95[0]:.3f}, {payload.credible_interval_95[1]:.3f})"
    )
    ...
    print(
        "✓ Audit Point 3.1 SATISFIED: Full traceability with posterior, necessity, snippets, SHA256"
    )
    print()
...
def example_2_quality_score_with_uncertainty():
    ...
    print(
        "✓ Audit Point 3.2 SATISFIED: Uncertainty-aware quality scores with credible intervals"
    )
    print()
...
def example_4_comparison_scenarios():
    ...
    for scenario in scenarios:
        ...
        print(f"  Interval Width: {quality['credible_interval_width']:.3f}")
        print(
            f"  Necessity: {'PASSED ✓' if scenario['necessity_passed'] else 'FAILED ✗'}"
        )
