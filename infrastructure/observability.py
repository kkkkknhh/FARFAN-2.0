...
class MetricsCollector:
    ...
    def increment(self, metric_name: str, tags: Optional[Dict[str, str]] = None) -> None:
        ...
        self.logger.debug(f"Counter incremented: {metric_name} {tags or {}} -> {self.counters[key]}")
    ...
    def _make_counter_key(self, metric_name: str, tags: Optional[Dict[str, str]] = None) -> str:
...
class ObservabilityStack:
    ...
    def record_pipeline_duration(self, duration_secs: float) -> None:
        ...
        if duration_secs > 1800:
            self.alert('HIGH', f'Pipeline duration exceeded 30min: {duration_secs:.1f}s')
    ...
    def record_hoop_test_failure(self, question: str, missing: List[str]) -> None:
        ...
        current_count = self.metrics_collector.get_count('pdm.evidence.hoop_test_fail_count')
        if current_count > 5:
            self.alert('HIGH', f'Multiple hoop test failures detected: {current_count} failures')
    ...
    def record_dimension_score(self, dimension: str, score: float) -> None:
        ...
        """
        self.metrics_collector.gauge(
            f'pdm.dimension.avg_score_{dimension}',
            score
        )
    ...
    @contextmanager
    def trace_operation(self, operation_name: str, **attributes) -> Generator[Span, None, None]:
        """
