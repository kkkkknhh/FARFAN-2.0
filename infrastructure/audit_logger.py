...
class ImmutableAuditLogger:
    ...
    def _load_existing_records(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to load existing audit records: {e}", exc_info=True
            )
    ...
    def append_record(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.error(f"Failed to persist audit record: {e}", exc_info=True)
    ...
    def get_recent_records(
        self, limit: int = 10, orchestrator: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if orchestrator:
            filtered_records = [
                r for r in filtered_records if r.orchestrator == orchestrator
            ]
    ...
    def get_records_by_run_id(self, run_id: str) -> List[Dict[str, Any]]:
        ...
        """
        return [r.to_dict() for r in self._records if r.run_id == run_id]
    ...
    def get_records_by_source(self, sha256_source: str) -> List[Dict[str, Any]]:
        ...
        """
        return [r.to_dict() for r in self._records if r.sha256_source == sha256_source]
