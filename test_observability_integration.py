...
class MockPDMOrchestrator:
    ...
    async def analyze_plan(self, pdf_path: str) -> dict:
        ...
        with self.observability.trace_operation(
            "analyze_plan", run_id=run_id, pdf=pdf_path
        ):
            # Phase I: Extraction
            ...
            with self.observability.trace_operation("bayesian_inference", phase="III"):
                ...
                if not convergence_ok:
                    self.observability.record_nonconvergent_chain(
                        "chain_2", "R_hat=1.12"
                    )
...
async def test_alert_behavior():
    ...
    orchestrator.observability.record_nonconvergent_chain(
        "chain_5", "Failed convergence"
    )
    print("   ✓ CRITICAL alert should have been logged above")
