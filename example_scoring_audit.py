...
from scoring_audit import EXPECTED_TOTAL_QUESTIONS, ScoringSystemAuditor
...
def create_sample_meso_report():
    ...
    return {
        "metadata": {"report_level": "MESO", "clusters": 4, "dimensions": 6},
        "clusters": {
            "C1": {
...
def create_sample_macro_report(global_score: float):
    ...
    return {
        ...
        "analisis_retrospectivo": {
            ...
            "debilidades": [
                "Teoría de cambio incompleta",
                "Métricas de impacto débiles",
            ],
        },
...
def main():
    ...
    print(
        f"   - Issues: {len([i for i in report.micro_issues if i.category == 'micro_scoring'])}"
    )
    ...
    if report.d6_scores_below_threshold:
        print(
            f"   - ⚠ WARNING: {len(report.d6_scores_below_threshold)} D6 questions need improvement"
        )
        for item in report.d6_scores_below_threshold[:3]:
            print(
                f"     • {item['question_id']}: {item['score']:.3f} (gap: {item['gap']:.3f})"
            )
    ...
    print(f"\n7. Overall Summary:")
    print(
        f"   - Overall Status: {'✓ VALID' if report.overall_valid else '✗ HAS ISSUES'}"
    )
    ...
    if report.critical_issues > 0:
        ...
        for issue in [
            ...
            for i in (
                report.micro_issues
                + report.meso_weight_issues
                + report.meso_convergence_gaps
                + report.macro_issues
                + report.rubric_issues
                + report.dnp_issues
            )
