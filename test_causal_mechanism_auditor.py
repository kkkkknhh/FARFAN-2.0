...
class TestCausalMechanismAuditor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph.add_node('PROG-001', type='programa', text='Diagnóstico de necesidades', responsible_entity='Secretaría de Planeación')
        self.graph.add_node('PROD-001', type='producto', text='Construcción de infraestructura', 
                           responsible_entity='Secretaría de Obras', financial_allocation=1000000,
                           entity_activity={'entity': 'Secretaría de Obras', 'activity': 'construcción', 'verb_lemma': 'construir'})
        self.graph.add_node('RES-001', type='resultado', text='Mejora en cobertura de servicios')
        self.graph.add_node('IMP-001', type='impacto', text='Desarrollo sostenible regional')
        
        # Add edges
        self.graph.add_edge('PROG-001', 'PROD-001', logic='para atender', keyword='para', 
                          posterior_mean=0.85, strength=0.85)
        self.graph.add_edge('PROD-001', 'RES-001', logic='mediante', keyword='mediante',
                          posterior_mean=0.75, strength=0.75)
        self.graph.add_edge('RES-001', 'IMP-001', logic='contribuye a', keyword='contribuye',
                          posterior_mean=0.65, strength=0.65)
        
        # Impossible transition (logical jump)
        self.graph.add_edge('PROD-001', 'IMP-001', logic='genera', keyword='genera',
                          posterior_mean=0.90, strength=0.90)
    ...
    def test_quality_grade_thresholds(self):
        ...
        self.assertEqual(
            self.auditor._get_necessity_quality_grade(1.0),
            QualityGrade.EXCELENTE
        )
        self.assertEqual(
            self.auditor._get_necessity_quality_grade(0.8),
            QualityGrade.BUENO
        )
        self.assertEqual(
            self.auditor._get_necessity_quality_grade(0.6),
            QualityGrade.REGULAR
        )
        self.assertEqual(
            self.auditor._get_necessity_quality_grade(0.3),
