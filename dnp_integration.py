...
try:
    ...
    from mga_indicadores import CATALOGO_MGA, IndicadorMGA, TipoIndicadorMGA
    ...
    logging.error(
        "Asegúrese de que competencias_municipales.py, mga_indicadores.py y pdet_lineamientos.py estén en el mismo directorio"
    )
...
class ValidadorDNP:
    ...
    def validar_proyecto_integral(
        ...
        resultado.competencias_validadas = competencias_result.get(
        ...
        score_mga = (
            40.0
            if resultado.cumple_mga
            else (20.0 if len(resultado.indicadores_mga_usados) > 0 else 0.0)
        )
        score_pdet = 0.0
        if self.es_municipio_pdet:
            score_pdet = (
                20.0
                if resultado.cumple_pdet
                else (10.0 if len(resultado.lineamientos_pdet_cumplidos) > 0 else 0.0)
            )
        else:
            ...
            score_mga = (
                50.0
                if resultado.cumple_mga
                else (25.0 if len(resultado.indicadores_mga_usados) > 0 else 0.0)
            )
    ...
    def _validar_competencias(self, sector: str, descripcion: str) -> Dict[str, Any]:
        ...
        return self.catalogo_competencias.validar_competencia_municipal(
            sector, descripcion
        )

    def _validar_indicadores_mga(
        self, indicadores_propuestos: List[str], sector: str
    ) -> Dict[str, Any]:
        ...
        recomendados_faltantes = [
            cod
            for cod in codigos_recomendados[:3]  # Top 3 recommendations
    ...
    def _validar_lineamientos_pdet(
        self,
        sector: str,
        es_rural: bool,
        poblacion_victimas: bool,
        lineamientos_cumplidos: List[str],
    ) -> Dict[str, Any]:
        ...
        cumplidos = [
            lin.codigo
            for lin in lineamientos_recomendados
            if lin.codigo in lineamientos_cumplidos
        ]
        pendientes = [
            lin.codigo
            for lin in lineamientos_recomendados
            if lin.codigo not in lineamientos_cumplidos
        ]

        cumple = (
            len(pendientes) == 0
        )  # True if all recommended guidelines are fulfilled

        recomendaciones = []
        if not cumple:
            if es_rural:
                recomendaciones.append(
                    "Fortalecer enfoque de desarrollo rural con enfoque territorial"
                )
            if poblacion_victimas:
                recomendaciones.append(
                    "Incluir componentes de reconciliación y reparación para víctimas"
                )
    ...
    def _generar_recomendaciones_generales(self, resultado: ResultadoValidacionDNP):
        ...
        if resultado.nivel_cumplimiento == NivelCumplimiento.INSUFICIENTE:
            resultado.recomendaciones.insert(
                0,
    ...
    def generar_reporte_cumplimiento(self, resultado: ResultadoValidacionDNP) -> str:
        ...
        lineas.append(
            f"NIVEL DE CUMPLIMIENTO: {resultado.nivel_cumplimiento.value.upper()}"
        )
        lineas.append("")
        ...
        lineas.append(
            f"   Estado: {'✓ CUMPLE' if resultado.cumple_competencias else '✗ NO CUMPLE'}"
        )
        if resultado.competencias_validadas:
            lineas.append(
                f"   Competencias aplicables: {', '.join(resultado.competencias_validadas[:3])}"
            )
        if resultado.competencias_fuera_alcance:
            lineas.append(
                f"   Fuera de alcance: {', '.join(resultado.competencias_fuera_alcance)}"
            )
        lineas.append("")
        ...
        lineas.append(
            f"   Estado: {'✓ CUMPLE' if resultado.cumple_mga else '✗ NO CUMPLE'}"
        )
        if resultado.indicadores_mga_usados:
            lineas.append(
                f"   Indicadores válidos: {', '.join(resultado.indicadores_mga_usados)}"
            )
        ...
        if resultado.es_municipio_pdet:
            ...
            lineas.append(
                f"   Estado: {'✓ CUMPLE' if resultado.cumple_pdet else '✗ NO CUMPLE'}"
            )
            if resultado.lineamientos_pdet_cumplidos:
                lineas.append(
                    f"   Cumplidos: {len(resultado.lineamientos_pdet_cumplidos)} lineamientos"
                )
            if resultado.lineamientos_pdet_pendientes:
                lineas.append(
                    f"   Pendientes: {len(resultado.lineamientos_pdet_pendientes)} lineamientos"
                )
            lineas.append("")
...
def validar_plan_desarrollo_completo(
    ...
    for proyecto in proyectos:
        ...
        resultados_proyectos.append(
            {"proyecto": proyecto.get("nombre", "Sin nombre"), "resultado": resultado}
        )

    # Aggregate results
    total_proyectos = len(proyectos)
    proyectos_cumplen = sum(
        1
        for r in resultados_proyectos
        if r["resultado"].nivel_cumplimiento
        in [NivelCumplimiento.EXCELENTE, NivelCumplimiento.BUENO]
    )

    score_promedio = (
        sum(r["resultado"].score_total for r in resultados_proyectos) / total_proyectos
    ...
    return {
        ...
        "tasa_cumplimiento": (
            (proyectos_cumplen / total_proyectos * 100) if total_proyectos > 0 else 0
        ),
