...
def demo_operationalization():
    ...
    print(f"  SOTA Compliance: {'YES ✓' if result_incomplete.sota_compliance else 'NO ✗'}")
    print(f"  Completeness Ratio: {result_incomplete.metrics['completeness_ratio']:.1%}")
...
def demo_quantified_gaps():
    ...
    print(f"  Quantified: {result.metrics['quantified_gaps']} ({result.metrics['quantification_ratio']:.1%})")
...
def demo_integrated():
    ...
    for audit_type, result in results.items():
        ...
        print(f"  {audit_type[:30].ljust(30)} | {severity_str} | {sota_str}     | {findings_count}")
