...
class TelemetryValidator:
    ...
    def validate_phase_boundaries(self, orchestrator) -> bool:
        ...
        for phase in phases:
            ...
            has_completion = any(
                e.event_type == EventType.PHASE_COMPLETION for e in events
            )
    ...
    def validate_telemetry_persistence(self, log_dir: Path) -> bool:
        ...
        for telemetry_file in telemetry_files:
            try:
                ...
                self.errors.append(
                    f"Failed to read telemetry file {telemetry_file}: {e}"
                )
                return False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        checks = [
            (
                ...
                lambda: validator.validate_telemetry_completeness(orch),
            ),
            ("Phase Boundaries", lambda: validator.validate_phase_boundaries(orch)),
            (
                "Trace Context Consistency",
                lambda: validator.validate_trace_context_consistency(orch),
            ),
            (
                "Deterministic Hashing",
                lambda: validator.validate_deterministic_hashing(),
            ),
            (
                "Audit Log Immutability",
                lambda: validator.validate_audit_log_immutability(orch, log_dir),
            ),
            (
                "Telemetry Persistence",
                lambda: validator.validate_telemetry_persistence(log_dir),
            ),
            (
                "Contract Enforcement",
                lambda: validator.validate_contract_enforcement(orch),
            ),
        ]
