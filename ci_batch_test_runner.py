...
class BatchTestRunner:
    ...
    def run_all_tests(self, verbose: bool = True) -> Tuple[bool, Dict]:
        ...
        for i, test_file in enumerate(test_files, 1):
            ...
            status = (
                "✓ PASSED" if result.failed == 0 and result.errors == 0 else "✗ FAILED"
            )
            print(
                f"  {status} - {result.passed}/{result.total_tests} tests passed "
        ...
        print(
            f"Status:        {'✓ ALL PASSED' if overall_success else '✗ FAILURES DETECTED'}"
        )
        print("=" * 80)
    ...
    def _run_module_tests(self, module_name: str, verbose: bool) -> ModuleTestResult:
        ...
        cmd = [sys.executable, "-m", "unittest", module_name, "-v" if verbose else ""]
    ...
    def _extract_detailed_failures(self, output: str, module_name: str) -> List[Dict]:
        ...
        for match in re.finditer(pattern, output, re.DOTALL):
            ...
            file_info = (
                f"{file_match.group(1)}:{file_match.group(2)}"
                if file_match
                else "unknown"
            )
    ...
    def _save_ci_results(
        self, results: Dict[str, ModuleTestResult], success: bool, duration: float
    ):
    ...
    def _detect_regressions(self, baseline: Dict, current: Dict) -> List[Dict]:
        ...
        for module_name in current_modules:
            ...
            baseline_failed = baseline_mod.get("failed", 0) + baseline_mod.get(
                "errors", 0
            )
    ...
    def _save_regressions(self, regressions: List[Dict]):
        ...
        data = {"timestamp": datetime.now().isoformat(), "regressions": regressions}
    ...
    def _print_regressions(self, regressions: List[Dict]):
        ...
        for i, reg in enumerate(regressions, 1):
            ...
            if reg["type"] == "new_failures":
                print(
                    f"   Failed tests increased: {reg['baseline_failed']} → {reg['current_failed']}"
                )
                print(f"   New failures: {reg['delta']}")
            elif reg["type"] == "performance_degradation":
                print(
                    f"   Duration increased: {reg['baseline_duration']:.2f}s → {reg['current_duration']:.2f}s"
                )
...
def main():
    ...
    parser.add_argument(
        "--set-baseline", action="store_true", help="Set current results as baseline"
    )
