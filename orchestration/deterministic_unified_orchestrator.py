...
class DeterministicUnifiedOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"Calibration: COHERENCE_THRESHOLD={self.calibration.COHERENCE_THRESHOLD}"
        )
    ...
    def _get_component(
        self, component_name: str, required: bool = True
    ) -> Optional[Any]:
        """
    ...
    def _validate_contract_inputs(
        self, phase: PipelinePhase, context: PipelineContext
    ) -> None:
        """
        ...
        if missing_inputs:
            raise ContractViolationError(
                ...
                context={
                    ...
                    "available_outputs": {
                        p.name: list(o.keys()) for p, o in context.phase_outputs.items()
                    },
                },
            )

    def _validate_contract_outputs(
        self, phase: PipelinePhase, outputs: Dict[str, Any]
    ) -> None:
        """
    ...
    async def execute_pipeline(
        self, pdf_path: str, plan_name: str = "PDM", dimension: str = "estratégico"
    ) -> PipelineContext:
        """
    ...
    async def _execute_phase(
        ...
        try:
            ...
            self._emit_telemetry(
                ...
                phase,
                context,
                {"phase": phase.name, "error": str(e), "error_type": type(e).__name__},
            )
    ...
    async def _execute_phase_2_statement_extraction(
        ...
        semantic_chunks = context.get_output(
            PipelinePhase.PHASE_1_EXTRACTION, "semantic_chunks", []
        )
        ...
        statements = [
            {
                "text": chunk["text"],
                "dimension": chunk.get("dimension", context.dimension),
            }
            for chunk in semantic_chunks
        ]

        return {"statements": statements, "statement_count": len(statements)}
    ...
    async def _execute_phase_3_causal_graph(
        ...
        statements = context.get_output(
            PipelinePhase.PHASE_2_STATEMENT_EXTRACTION, "statements", []
        )
    ...
    async def _execute_phase_4_contradiction_detection(
        ...
        contradiction_detector = self._get_component(
            "contradiction_detector", required=True
        )
    ...
    async def _execute_phase_5_bayesian_inference(
        ...
        causal_graph = context.get_output(
            PipelinePhase.PHASE_3_CAUSAL_GRAPH, "causal_graph"
        )
        ...
        return {"mechanism_results": mechanism_results, "posteriors": posteriors}
    ...
    async def _execute_phase_6_regulatory_validation(
        ...
        dnp_validation_result = {"passed": True, "warnings": []}
    ...
    async def _execute_phase_7_axiomatic_validation(
        ...
        axiomatic_validation_result = {"passed": True, "warnings": []}
    ...
    async def _execute_phase_8_quality_scoring(
        ...
        # Get inputs from previous phases
        mechanism_results = context.get_output(
            PipelinePhase.PHASE_5_BAYESIAN_INFERENCE, "mechanism_results", []
        )
        contradictions = context.get_output(
            PipelinePhase.PHASE_4_CONTRADICTION_DETECTION, "contradictions", []
        )
        axiomatic_validation_result = context.get_output(
            PipelinePhase.PHASE_7_AXIOMATIC_VALIDATION,
            "axiomatic_validation_result",
            {},
        )
    ...
    async def _execute_phase_9_recommendation_generation(
        ...
        smart_recommendation = self._get_component(
            "smart_recommendation", required=False
        )
        ...
        quality_grade = context.get_output(
            PipelinePhase.PHASE_8_QUALITY_SCORING, "quality_grade", "Unknown"
        )
