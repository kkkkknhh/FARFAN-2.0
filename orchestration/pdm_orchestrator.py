    ...
    def __post_init__(self):
        if not self.dimension_scores:
            self.dimension_scores = {
                'D1': 0.7, 'D2': 0.7, 'D3': 0.7, 
                'D4': 0.7, 'D5': 0.7, 'D6': 0.7
            }
...
class AnalysisResult:
    ...
    def to_audit_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'graph_nodes': self.causal_graph.number_of_nodes() if self.causal_graph else 0,
            'graph_edges': self.causal_graph.number_of_edges() if self.causal_graph else 0,
...
class MetricsCollector:
    ...
    def increment(self, counter_name: str) -> None:
        ...
        self.logger.debug(f"Counter incremented: {counter_name}={self.counters[counter_name]}")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            'metrics': {k: {'count': len(v), 'last': v[-1] if v else 0} for k, v in self.metrics.items()},
...
class PDMOrchestrator:
    ...
    def __init__(self, config: Any):
        ...
        if hasattr(config, 'self_reflection') and hasattr(config.self_reflection, 'prior_history_path'):
            audit_store_path = Path(config.self_reflection.prior_history_path).parent / "audit_logs.jsonl"
    ...
    async def _execute_phases(
        ...
        if extraction.extraction_quality.get('score', 1.0) < min_quality_threshold:
            raise DataQualityError(f"Extraction quality too low: {extraction.extraction_quality}")
        
        # PHASE II: DAG Construction (Core Synthesis)
        self._transition_state(PDMAnalysisState.BUILDING_DAG)
        causal_graph = await self._build_graph(extraction.semantic_chunks, extraction.tables)
        ...
        validation_task = asyncio.create_task(
            self._validate_complete(causal_graph, extraction.semantic_chunks, extraction.tables)
        )
        ...
        self.metrics.record(
            ...
            self._compute_prior_decay(mechanism_results)
        )
        self.metrics.record(
            'evidence.hoop_test_fail_count',
            sum(1 for r in mechanism_results if not r.necessity_test.get('passed', True))
        )
        ...
        return AnalysisResult(
            ...
            mechanism_results=mechanism_results,
            validation_results=validation_results,
            recommendations=self._generate_recommendations(final_score, validation_results)
        )
    ...
    async def _build_graph(self, semantic_chunks: List[Dict], tables: List[Dict]) -> Any:
    ...
    async def _infer_all_mechanisms(self, causal_graph: Any, semantic_chunks: List[Dict]) -> List[MechanismResult]:
        """Phase III: Infer mechanisms"""
        if self.bayesian_engine:
            return await self.bayesian_engine.infer_all_mechanisms(causal_graph, semantic_chunks)
        
        # Fallback
        self.logger.warning("No bayesian_engine configured, using fallback")
        return [MechanismResult(type='fallback', necessity_test={'passed': True, 'missing': []})]
    
    async def _validate_complete(self, causal_graph: Any, semantic_chunks: List[Dict], 
                                 tables: List[Dict]) -> ValidationResult:
        """Phase III: Validate complete"""
        if self.validator:
            return await self.validator.validate_complete(causal_graph, semantic_chunks, tables)
    ...
    def _calculate_quality_score(
        ...
        return QualityScore(
            ...
            dimension_scores={'D1': overall, 'D2': overall, 'D3': overall, 
                            'D4': overall, 'D5': overall, 'D6': overall}
        )
    ...
    def _generate_recommendations(self, final_score: QualityScore, 
                                 validation_results: ValidationResult) -> List[str]:
        """Generate recommendations based on results"""
        recommendations = []
        
        if final_score.overall_score < 0.6:
            recommendations.append("Overall quality score is below acceptable threshold (0.6)")
        
        for dim, score in final_score.dimension_scores.items():
            if score < 0.55:
                recommendations.append(f"Dimension {dim} score ({score:.2f}) is critically low")
        
        if not validation_results.passed:
            recommendations.append("Validation failed - review validation warnings")
        
        if validation_results.warnings:
            recommendations.append(f"Address {len(validation_results.warnings)} validation warnings")
    ...
    def _handle_timeout(self, run_id: str, pdf_path: str) -> AnalysisResult:
        ...
        return AnalysisResult(
            ...
            recommendations=["Analysis timed out - consider increasing worker_timeout_secs"]
        )
    
    def _handle_failure(self, run_id: str, pdf_path: str, error: Exception) -> AnalysisResult:
