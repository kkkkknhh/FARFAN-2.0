...
from dnp_integration import NivelCumplimiento, ResultadoValidacionDNP, ValidadorDNP
...
class TestRegulatoryValidationDeterminism:
    ...
    def test_identical_input_produces_identical_output(self):
        ...
        assert reg1.get("metrics", {}).get("score_raw") == reg2.get("metrics", {}).get(
            "score_raw"
        )
        assert reg1.get("metrics", {}).get("score_adjusted") == reg2.get(
            "metrics", {}
        ).get("score_adjusted")
        assert reg1.get("metrics", {}).get("cumple_competencias") == reg2.get(
            "metrics", {}
        ).get("cumple_competencias")
        assert reg1.get("metrics", {}).get("cumple_mga") == reg2.get("metrics", {}).get(
            "cumple_mga"
        )
    ...
    def test_score_range_contract(self):
        ...
        for text in test_cases:
            ...
            if reg.get("status") == "success":
                ...
                assert 0.0 <= score_raw <= 1.0, (
                    f"score_raw {score_raw} out of range for text: {text[:50]}"
                )
                assert 0.0 <= score_adjusted <= 1.0, (
                    f"score_adjusted {score_adjusted} out of range for text: {text[:50]}"
                )
    ...
    def test_sector_detection(self):
        ...
        for text, expected_sector in test_cases:
            ...
            assert detected_sector == expected_sector, (
                f"Expected {expected_sector}, got {detected_sector} for text: {text}"
            )
...
class TestRegulatoryValidationAuditTrail:
    ...
    def test_full_audit_trail(self):
        ...
        result = orch.orchestrate_analysis(
            "Plan educativo con EDU-001 para zona rural", "PDM_Audit", "estratégico"
        )
    ...
    def test_traceability_to_dnp_standards(self):
        ...
        result = orch.orchestrate_analysis(
            "Construcción educativa con EDU-001", "PDM_Trace", "estratégico"
        )
...
class TestRegulatoryValidationEdgeCases:
    ...
    def test_no_mga_indicators(self):
        ...
        result = orch.orchestrate_analysis(
            "Plan de desarrollo sin indicadores específicos", "PDM_NoMGA", "estratégico"
        )
    ...
    def test_unknown_sector(self):
        ...
        result = orch.orchestrate_analysis(
            "Proyecto de quantum computing blockchain", "PDM_Unknown", "estratégico"
        )
    ...
    def test_validador_dnp_scoring(self):
        ...
        resultado = validador.validar_proyecto_integral(
            ...
            indicadores_propuestos=[
                "EDU-001",
                "EDU-020",
            ],  # EDU-020 is PRODUCTO, EDU-001 is RESULTADO
        ...
        assert (
            len(resultado.indicadores_mga_usados) > 0
            or len(resultado.indicadores_mga_faltantes) > 0
        )
