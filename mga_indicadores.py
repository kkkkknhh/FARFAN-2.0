...
class IndicadorMGA:
    ...
    nivel_aplicacion: List[str] = field(
...
class CatalogoIndicadoresMGA:
    ...
    def buscar_indicador_por_descripcion(self, terminos: str) -> List[IndicadorMGA]:
        ...
        for ind in self.indicadores.values():
            if (
                terminos_lower in ind.nombre.lower()
                or terminos_lower in ind.definicion.lower()
            ):
    ...
    def generar_reporte_alineacion(self, codigos_usados: List[str]) -> Dict[str, Any]:
        ...
        indicadores_usados = [
            self.get_indicador(cod) for cod in codigos_usados if self.get_indicador(cod)
        ]

        return {
            "total_indicadores_usados": len(indicadores_usados),
            "productos": len(
                [i for i in indicadores_usados if i.tipo == TipoIndicadorMGA.PRODUCTO]
            ),
            "resultados": len(
                [i for i in indicadores_usados if i.tipo == TipoIndicadorMGA.RESULTADO]
            ),
            "sectores_cubiertos": list({i.sector for i in indicadores_usados}),
            "ods_relacionados": list(
                {ods for i in indicadores_usados for ods in i.ods_relacionados}
            ),
