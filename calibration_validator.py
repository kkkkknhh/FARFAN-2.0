...
def main():
    ...
    report = {
        "validation_summary": {
            ...
            "stability_rate": sum(1 for r in all_results if r.is_stable) / len(all_results) * 100
        },
    ...
    if math.isclose(report["validation_summary"]["stability_rate"], 100.0, rel_tol=1e-9, abs_tol=1e-12) and prior_valid:  # replaced float equality with isclose (tolerance from DEFAULT_FLOAT_TOLS)
        ...
        print(f"\n✗ {report['validation_summary']['unstable_constants']} constant(s) UNSTABLE", file=sys.stderr)
        return 1
